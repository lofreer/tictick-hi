...
func addDataContractPaths(paths map[string]apiPathItem) {
	...
	addOperation(paths, "/api/data/tasks/{id}/repair-invalid-issues", http.MethodPost, operation(
		"data", "repairDataSyncTaskInvalidIssues", "Create sync tasks for detected invalid candles", http.StatusOK, schemaRef("DataSyncGapRepairResult"),
		withCSRF(), withParameters(pathParam("id", "Data sync task id")), withRequest(schemaRef("RepairDataSyncInvalidIssuesRequest")), withErrors(http.StatusBadRequest, http.StatusNotFound),
	))
...
func addMarketContractPaths(paths map[string]apiPathItem) {
	...
	addOperation(paths, "/api/market/candle-invalid-issues/quarantine", http.MethodPost, operation(
		"market", "quarantineMarketCandleInvalidIssues", "Archive and remove persisted invalid open-time candles", http.StatusOK, schemaRef("MarketCandleQuarantineResult"),
		withCSRF(), withRequest(schemaRef("QuarantineMarketCandleInvalidIssuesRequest")), withErrors(http.StatusBadRequest, http.StatusNotFound),
	))
...
func marketInstrumentSyncResultSchema() map[string]any {
	return map[string]any{
		"type":     "object",
		"required": []string{"exchange", "activeCount", "inactiveCount", "pausedDataSyncTaskCount", "restoredDataSyncTaskCount", "syncedAt"},
		"properties": map[string]any{
			...
			"restoredDataSyncTaskCount": map[string]any{"type": "integer"},
