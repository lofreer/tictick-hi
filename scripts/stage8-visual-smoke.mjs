...
function assertResearchChartSmoke(label, sample, viewport) {
  ...
  assertChartViewportSmoke(label, sample, viewport, 600);
}
...
function assertChartViewportSmoke(label, sample, viewport, desktopMinimumHeight) {
  ...
  const minimumHeight = viewport.width <= 760 ? 540 : viewport.width <= 980 ? 620 : desktopMinimumHeight;
  ...
  assertAxisTextInk(label, "right price-axis", sample.priceAxisTextInk, axisLabelInkMinimum(viewport), axisLabelInkMaximum(viewport), {
    priceAxis: sample.priceAxisCanvas,
    chartViewport: sample.chartViewport,
  });
...
function axisLabelInkMaximum(viewport) {
  return viewport.width <= 760 ? maxMobileAxisLabelInkHeight : maxAxisLabelInkHeight;
}
...
function assertDetailLayoutSmoke(label, sample, viewport) {
  ...
  if (viewport.width > 980) {
    ...
    if (sample.detail.summary.rectWidth < 220 || sample.detail.summary.rectWidth > 280) {
