...
docker run --rm \
  ...
  go test ./internal/store/postgres -run 'TestIntegration(DataSyncRunnerRecoversAfter(BinanceRetryAfter|OKXRateLimitCode)|DataSyncExchangeBackoffDoesNotBlockOtherExchanges)' -count=1 -v
