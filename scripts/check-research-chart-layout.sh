...
require_contains "$RESEARCH_CSS" "max-height: clamp(176px, 21vh, 232px);"
...
require_block_contains "$RESEARCH_CSS" ".research-toolbar" "padding: 9px 12px;"
...
require_contains "$THEME_TOKENS" "export const chartAxisFontSize = 13;"
require_contains "$THEME_TOKENS" "export const chartMobileAxisFontSize = 13;"
require_contains "$THEME_TOKENS" "desktop: 56,"
require_contains "$THEME_TOKENS" "narrowDesktop: 56,"
require_contains "$THEME_TOKENS" "mobile: 54,"
require_contains "$QUALITY_AUDIT" '坐标轴字号收敛为桌面/窄桌面/移动端 `13px`'
require_contains "$QUALITY_AUDIT" '右侧价格轴最大宽度 `72px`'
require_contains "$QUALITY_AUDIT" '坐标轴文字墨迹高度范围为桌面/窄桌面/移动端 `8px` 到 `15px`'
...
require_contains "$CHART_VUE" "priceFormat: {"
require_contains "$CHART_VUE" 'type: "custom",'
require_contains "$CHART_VUE" "formatter: formatChartPrice,"
require_not_contains "$CHART_VUE" "new MutationObserver(handleChartMutations)"
require_not_contains "$CHART_VUE" "repairDistortedChartCanvases"
require_not_contains "$CHART_VUE" "function repairDistortedCanvases()"
...
require_contains "$CHART_SMOKE" "'.tv-lightweight-charts'"
require_not_contains "$CHART_SMOKE" "'.tv-lightweight-charts tbody'"
require_not_contains "$CHART_SMOKE" "'.tv-lightweight-charts tr'"
require_not_contains "$CHART_SMOKE" "'.tv-lightweight-charts td'"
require_not_contains "$CHART_SMOKE" "element.style.width = '9000px';"
