...
require_block_contains "$RESEARCH_CSS" ".research-source-controls" "grid-template-columns: 100px 120px 28px 56px max-content;"
...
require_block_contains "$RESEARCH_CSS" ".research-toolbar" "grid-template-columns: minmax(0, auto) minmax(280px, 1fr);"
require_block_contains "$RESEARCH_CSS" ".research-toolbar" "padding: 6px 10px;"
...
require_contains "$RESEARCH_CSS" "--kline-chart-plot-height: clamp(620px, 66dvh, 760px);"
...
require_contains "$RESEARCH_CSS" "width: 120px;"
require_contains "$RESEARCH_CSS" "max-width: min(240px, 30vw);"
