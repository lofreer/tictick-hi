...
describe("TradingViewChart", () => {
  ...
  it("formats price labels as full values without compact shorthand", () => {
    ...
    expect(chartMocks.addSeries).toHaveBeenCalledWith("CandlestickSeries", expect.objectContaining({ priceFormat: { formatter: options.localization.priceFormatter, minMove: 0.00000001, type: "custom" } }));
  ...
  it("pads the visible logical range so edge time labels are not clipped", () => {
    ...
    expect(chartMocks.setVisibleLogicalRange).toHaveBeenCalledWith({ from: -1, to: 1 });
  ...
  it("scales the logical edge padding with dense candle windows", () => {
    ...
    expect(chartMocks.setVisibleLogicalRange).toHaveBeenLastCalledWith({ from: 711, to: 1007 });
