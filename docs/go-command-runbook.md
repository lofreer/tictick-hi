...
- structured text / JSON log output, log level config, command run-level correlation IDs, API `X-Request-ID` response headers, API access logs with `request_id`, and API-created data sync / backtest / trading task `requestId` fields exist, but trace IDs are not propagated into repair-derived tasks, notification delivery, external systems, or subcommands;
