...
| 登录会话 | demo | 保留后加强 | HttpOnly session cookie、CSRF double-submit 写保护、登录失败持久化节流、当前操作员 session 列表、基础来源地址 / User-Agent 上下文、非当前 session 撤销、基础密码长度 / 复杂度 / 弱密码字典 / 不含用户名策略、当前操作员自禁用保护和最后启用操作员保护已进入 API / 系统管理边界；登录成功 / 失败、退出和会话撤销会进入基础操作审计；仍缺更完整弱密码字典 / 历史 / 轮换、RBAC / 角色级自保护规则和生产级设备上下文 |
...
- 交易所账号密钥 digest 风险已在阶段 6 切片关闭到 `demo`；历史非 AES-GCM 行标记为 `legacy`。
- live executor 仍禁用，testnet/sandbox、幂等提交、真实交易所提交和生产密钥管理仍未建立。
- 登录会话基础审计、持久化限流、基础来源上下文、基础密码长度 / 复杂度 / 弱密码字典 / 不含用户名策略、当前操作员自禁用保护和最后启用操作员保护已在后续补充关闭到 demo 边界；更完整弱密码字典 / 历史 / 轮换、RBAC / 角色级自保护规则和生产级设备上下文仍未完成。
...
- 这只是基础 session 管理，不是生产级登录安全；持久化限流、基础来源上下文、基础密码长度 / 复杂度 / 弱密码字典 / 不含用户名策略、当前操作员自禁用保护和最后启用操作员保护已在后续补充关闭到 demo 边界，仍缺更完整弱密码字典 / 历史 / 轮换、RBAC / 角色级自保护、设备指纹 / 变更告警和生产级审计。
...
| 登录会话 | demo | HttpOnly session、CSRF double-submit、登录失败持久化节流、session 列表 / 来源上下文和撤销有 route / smoke 覆盖；登录成功 / 失败、退出、session 撤销已进入基础操作审计；基础密码长度 / 复杂度 / 弱密码字典 / 不含用户名策略、当前操作员自禁用保护和最后启用操作员保护已覆盖 | 无更完整弱密码字典/历史/轮换/RBAC、角色级自保护规则和生产级设备上下文 |
