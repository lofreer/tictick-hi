...
| 数据同步 worker | demo | claim/heartbeat/upsert/retry/release、临时错误 `next_attempt_at` 持久化退避、失败后 UI retry、Stage 8 smoke 和容器 SIGTERM smoke 有覆盖 | 未证明真实交易所网络下长期恢复、交易所级全局限流和完整状态机 |
