...
补充：阶段 1 研究页、回测详情和交易详情的 K 线图表布局在 2026-06-30 继续收紧；当前有效布局约束以 `klineChartLayout.css`、`ResearchPage.css`、`detailChartLayout.css`、`scripts/stage8-visual-smoke.mjs` 和 `scripts/research-chart-height-smoke.mjs` 为准：研究页主工具栏 symbol 输入为桌面/窄桌面 `96px`、移动端 `92px`，主工具栏不再显示 symbol 内置 instrument sync 按钮，桌面工具栏采用左侧 market strip + 右侧单行可滚动状态摘要的一行工作台布局，窄屏再堆叠；图表左/右 gutter 为桌面 `14px/2px`、窄桌面 `12px/2px`、移动端 `10px/2px`；plot 高度为桌面 `clamp(680px, 72dvh, 820px)`、窄桌面 `700px`、移动端 `580px`，上下 padding 归零；价格轴可读性以后一段和“坐标轴字号恢复到正常尺寸补充”为准：坐标轴字体桌面/窄桌面 `28px`、移动端 `25px`，右侧价格轴 minimumWidth 为桌面 `128px`、窄桌面 `120px`、移动端 `104px`，价格标签保持完整数值显示；visual smoke 同时断言 symbol 最大宽度 `100px`、工具栏控件最大宽度 `500px`、工具栏高度最大 `72px`、右侧价格轴最大宽度 `140px`、坐标轴文字墨迹高度桌面/窄桌面不低于 `20px`、移动端不低于 `18px`、主图 canvas 右边界贴住右侧价格轴左边界、最右侧 canvas 贴住 viewport 右边界，详情页下方摘要列保持 `minmax(220px, 260px)`。
...
补充：阶段 1 K 线图表坐标轴真实可读性在 2026-07-01 再次收紧；本段覆盖前述 `28px/25px` 和墨迹高度 `20px/18px` 旧合同。Headless Chrome 复核显示 chart option `28px` 在实际 canvas 中只有约 `20px` 文字墨迹，仍会显得偏小；共享 `TradingViewChart` 坐标轴字号提升为桌面/窄桌面 `32px`、移动端 `29px`，覆盖研究页、回测详情和交易详情；右侧价格轴 minimumWidth 仍保持桌面 `128px`、窄桌面 `120px`、移动端 `104px`，且 visual smoke 继续保留右侧价格轴 `140px` 最大宽度，主图宽度占比下限按该右轴上限校准为桌面 `0.898`、窄桌面 `0.815`、移动端 `0.63`，避免重新制造右侧空白；时间轴边缘 padding 提升到 `30px` 到 `40px`，年份刻度使用两位年份，避免大字号时间标签贴住固定图表槽边缘；`scripts/stage8-visual-smoke.mjs` 和 `scripts/research-chart-height-smoke.mjs` 的坐标轴文字墨迹高度下限提升为桌面/窄桌面 `23px`、移动端 `21px`。该证据只证明坐标轴可读性合同进一步收紧，不代表图表研究能力已经达到 usable 或具备像素快照基线。

补充：阶段 1 K 线图表坐标轴正常字号在 2026-07-01 继续收紧；本段覆盖前述 `32px/29px` 和墨迹高度 `23px/21px` 旧合同。共享 `TradingViewChart` 坐标轴字号提升为桌面/窄桌面 `42px`、移动端 `32px`，覆盖研究页、回测详情和交易详情；右侧价格轴 minimumWidth 调整为桌面 `158px`、窄桌面 `150px`、移动端 `118px`，继续保持完整价格数值显示且不恢复 `k/K/m/M/b/B` 缩写；visual smoke 右侧价格轴最大宽度 `184px`，主图宽度占比下限按新右轴预算校准为桌面 `0.86`、窄桌面 `0.74`、移动端 `0.59`，同时坐标轴文字墨迹高度下限提升为桌面/窄桌面 `28px`、移动端 `23px`。该证据只收紧 K 线图表可读性合同，不代表图表研究能力已经达到 usable 或具备像素快照基线。

补充：阶段 1 K 线图表坐标轴正常字号在 2026-07-01 继续收紧；本段覆盖前述 `42px/32px` 和墨迹高度 `28px/23px` 旧合同。共享 `TradingViewChart` 坐标轴字号提升为桌面/窄桌面 `46px`、移动端维持 `32px`，覆盖研究页、回测详情和交易详情；右侧价格轴 minimumWidth 调整为桌面 `164px`、窄桌面 `156px`、移动端维持 `118px`，继续保持完整价格数值显示且不恢复 `k/K/m/M/b/B` 缩写；时间轴边缘 padding 提升到 `40px` 到 `56px`，避免大字号时间标签贴住固定图表槽边缘；visual smoke 右侧价格轴最大宽度 `196px`，同时坐标轴文字墨迹高度下限提升为桌面/窄桌面 `31px`、移动端维持 `23px`。已执行 `pnpm --dir web/frontend exec vitest run src/components/chart/TradingViewChart.test.ts src/pages/ResearchPage.layout.test.ts src/pages/DetailPages.layout.test.ts`、`pnpm --dir web/frontend run build`、`BASE_URL=http://127.0.0.1:18080 SMOKE_SAMPLES=4 SMOKE_INTERVAL_MS=120 SMOKE_SETTLE_MS=800 SMOKE_TOTAL_TIMEOUT_MS=180000 node scripts/research-chart-height-smoke.mjs`、`BASE_URL=http://127.0.0.1:18080 SMOKE_SETTLE_MS=800 SMOKE_TOTAL_TIMEOUT_MS=300000 node scripts/stage8-visual-smoke.mjs`、`pnpm --dir web/frontend run test`、`scripts/quality-gate.sh`、`go test ./...`、`go vet ./...` 和 `git diff --check`。该证据只收紧 K 线图表可读性合同，不代表图表研究能力已经达到 usable 或具备像素快照基线。

补充：阶段 1 K 线图表坐标轴正常字号在 2026-07-01 继续收紧；本段覆盖前述 `46px/32px` 和墨迹高度 `31px/23px` 旧合同。共享 `TradingViewChart` 坐标轴字号提升为桌面/窄桌面 `56px`、移动端提升为 `36px`，覆盖研究页、回测详情和交易详情；右侧价格轴 minimumWidth 调整为桌面 `208px`、窄桌面 `196px`、移动端 `132px`，继续保持完整价格数值显示且不恢复 `k/K/m/M/b/B` 缩写；时间轴边缘 padding 提升到 `128px` 到 `180px`，避免大字号时间标签贴住固定图表槽边缘；visual smoke 右侧价格轴最大宽度 `244px`，主图宽度占比下限按新右轴预算校准为桌面 `0.82`、窄桌面 `0.68`、移动端 `0.55`，坐标轴 band 高度上限校准为 `136px`，同时坐标轴文字墨迹高度下限提升为桌面/窄桌面 `38px`、移动端提升为 `26px`。已执行 `pnpm --dir web/frontend exec vitest run src/components/chart/TradingViewChart.test.ts src/pages/ResearchPage.layout.test.ts src/pages/DetailPages.layout.test.ts`、`pnpm --dir web/frontend run build`、`BASE_URL=http://127.0.0.1:8080 SMOKE_SAMPLES=3 SMOKE_INTERVAL_MS=100 SMOKE_SETTLE_MS=800 SMOKE_TOTAL_TIMEOUT_MS=180000 node scripts/research-chart-height-smoke.mjs`、`BASE_URL=http://127.0.0.1:8080 SMOKE_SETTLE_MS=800 SMOKE_TOTAL_TIMEOUT_MS=300000 node scripts/stage8-visual-smoke.mjs`、`pnpm --dir web/frontend run test`、`scripts/quality-gate.sh`、`go test ./...`、`go vet ./...` 和 `git diff --check`。该证据只收紧 K 线图表可读性合同，不代表图表研究能力已经达到 usable 或具备像素快照基线。

补充：阶段 1 K 线图表坐标轴在 2026-07-01 回到正常交易终端比例；本段覆盖前述 `56px/36px`、右侧轴 `208/196/132px`、`20px/18px` 和右侧轴 `100/94/88px` 旧合同。参考 `tictickbot` / `tictick-lite` 现有 K 线图表未使用夸张轴字号的基准，共享 `TradingViewChart` 坐标轴字号回到桌面/窄桌面/移动端 `12px`，覆盖研究页、回测详情和交易详情；右侧价格轴 minimumWidth 调整为桌面 `68px`、窄桌面 `64px`、移动端 `60px`，继续完整显示价格数值且不恢复 `k/K/m/M/b/B` 缩写；时间轴边缘 padding 保持 `24px` 到 `48px`，避免右侧留白被时间轴 padding 放大；visual smoke 右侧价格轴最大宽度 `84px`，主图宽度占比下限校准为桌面 `0.90`、窄桌面 `0.82`、移动端 `0.70`，坐标轴 band 高度上限 `72px`，坐标轴文字墨迹高度下限为桌面/窄桌面/移动端 `9px`。已执行 `pnpm --dir web/frontend exec vitest run src/components/chart/TradingViewChart.test.ts src/pages/ResearchPage.layout.test.ts src/pages/DetailPages.layout.test.ts`、`pnpm --dir web/frontend run typecheck`、`pnpm --dir web/frontend run test`、`pnpm --dir web/frontend run build`、`go test ./...`、`go vet ./...`、`scripts/check-file-size.sh`、`scripts/check-research-chart-layout.sh`、`scripts/quality-gate.sh`、`git diff --check`、`docker compose up -d --build api`、`curl -fsS http://127.0.0.1:8080/readyz`、`BASE_URL=http://127.0.0.1:8080 CHROME_REMOTE_DEBUGGING_PORT=9231 SMOKE_SAMPLES=4 SMOKE_INTERVAL_MS=120 SMOKE_SETTLE_MS=1000 SMOKE_TOTAL_TIMEOUT_MS=180000 node scripts/research-chart-height-smoke.mjs` 和 `BASE_URL=http://127.0.0.1:8080 CHROME_REMOTE_DEBUGGING_PORT=9241 SMOKE_SETTLE_MS=800 SMOKE_TOTAL_TIMEOUT_MS=300000 node scripts/stage8-visual-smoke.mjs`。该证据只恢复正常图表比例，不改变 CandleProvider、data sync、回测、交易或实盘能力等级。

补充：阶段 1 K 线图表坐标轴正常范围在 2026-07-01 继续收敛；本段覆盖前述 `12px` 下限合同和 `56px/36px` 巨型字号旧合同。参考 `tictickbot` / `tictick-lite` 未显式放大 lightweight-charts 轴字号的比例，当前有效合同为共享 `TradingViewChart` 坐标轴字号收敛为桌面/窄桌面 `14px`、移动端 `13px`，右侧价格轴 minimumWidth 为桌面 `78px`、窄桌面 `74px`、移动端 `68px`，价格 formatter 继续输出完整数值且禁止 `k/K/m/M/b/B` 缩写；visual smoke 仍要求右侧价格轴最大宽度 `84px`、主图宽度占比下限桌面 `0.90` / 窄桌面 `0.82` / 移动端 `0.70`，并把坐标轴文字墨迹高度范围为桌面/窄桌面 `9px` 到 `16px`、移动端 `9px` 到 `14px`，过小和过大的坐标轴都会失败。该证据只修正 K 线图表视觉合同，不改变 CandleProvider、data sync、回测、交易或实盘能力等级。
