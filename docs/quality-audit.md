...
- `TradingViewChart` 使用更紧凑的 chart 字体，右侧价格轴响应式最小宽度收敛为 `40/44/48px`。
- `stage8-visual-smoke.mjs` 增加 `narrow-desktop-812x1320`，并断言图表左/右 gutter 与 CSS 配置一致，右侧 gutter 必须保持紧凑。
- `research-chart-height-smoke.mjs` 和 visual smoke 的右侧价格轴上限统一收紧为 `68px`。
- 本地 `8080` 必须重建 API 镜像后用真实生产静态产物验证。

修复范围：

- `web/frontend/src/pages/ResearchPage.css` 将图表 body padding 拆为 left/right 变量，桌面右侧收紧到 `8px`，窄桌面/移动收紧到 `6px`。
...
- `web/frontend/src/components/chart/TradingViewChart.vue` 将响应式右价格轴最小宽度改为 `40/44/48px`。
