...
补充：阶段 1 研究页、回测详情和交易详情的 K 线图表布局在 2026-07-01 以 `tictickbot` / `tictick-lite` 的正常 K 线比例重新校准；当前有效布局约束以 `klineChartLayout.css`、`ResearchPage.css`、`detailChartLayout.css`、`scripts/stage8-visual-smoke.mjs` 和 `scripts/research-chart-height-smoke.mjs` 为准：研究页主工具栏 symbol 输入为桌面/窄桌面 `96px`、移动端 `92px`，桌面工具栏采用左侧 market strip + 右侧单行可滚动状态摘要的一行工作台布局，窄屏再堆叠；图表左/右 gutter 为桌面 `14px/2px`、窄桌面 `12px/2px`、移动端 `10px/2px`；plot 高度为桌面 `clamp(680px, 72dvh, 820px)`、窄桌面 `700px`、移动端 `580px`，上下 padding 归零；共享 `TradingViewChart` 坐标轴字号收敛为桌面/窄桌面/移动端 `12px`，右侧价格轴 minimumWidth 为桌面/窄桌面 `52px`、移动端 `50px`，价格 formatter 继续输出完整数值且禁止 `k/K/m/M/b/B` 缩写；visual smoke 断言 symbol 最大宽度 `100px`、工具栏控件最大宽度 `500px`、工具栏高度最大 `72px`、右侧价格轴最大宽度 `72px`、坐标轴文字墨迹高度范围为桌面/窄桌面/移动端 `7px` 到 `13px`、canvas CSS scale 不能被明显拉伸/压缩、主图 canvas 右边界贴住右侧价格轴左边界、最右侧 canvas 贴住 viewport 右边界，详情页下方摘要列保持 `minmax(220px, 260px)`。

补充：阶段 1 K 线图表坐标轴正常范围在 2026-07-01 继续收敛；本段覆盖前述过小和巨型字号旧合同。参考 `tictickbot` / `tictick-lite` 未显式放大 lightweight-charts 轴字号的比例，当前有效合同为共享 `TradingViewChart` 坐标轴字号收敛为桌面/窄桌面/移动端 `12px`，右侧价格轴 minimumWidth 为桌面/窄桌面 `52px`、移动端 `50px`，价格 formatter 继续输出完整数值且禁止 `k/K/m/M/b/B` 缩写；visual smoke 仍要求右侧价格轴最大宽度 `72px`、主图宽度占比下限桌面 `0.90` / 窄桌面 `0.82` / 移动端 `0.70`，并把坐标轴文字墨迹高度范围为桌面/窄桌面/移动端 `7px` 到 `13px`，过小和过大的坐标轴都会失败，canvas CSS scale 被明显拉伸/压缩也会失败。该证据只修正 K 线图表视觉合同，不改变 CandleProvider、data sync、回测、交易或实盘能力等级。

补充：阶段 1 K 线图表内部缩放回归在 2026-07-01 继续收紧；`.tv-lightweight-charts` 根节点不再由外部 CSS 强制 `width/height:100%!important`，只保留最大尺寸约束和外层 fixed viewport 裁剪，避免第三方内部 canvas 被外部 100% 缩放拉伸成巨型坐标轴。`scripts/stage8-visual-smoke.mjs` 和 `scripts/research-chart-height-smoke.mjs` 改为先按最右侧纵向 canvas 识别价格轴，再断言价格轴宽度、轴文字墨迹高度、canvas CSS scale 和主图/价格轴贴合关系；内部高度污染后的稳定性检查收敛到 document、panel、chart body、chart 和 canvas，正常首屏仍检查 `.tv-lightweight-charts` 与固定槽贴合。已执行本地 8080 图表高度 smoke 和 stage8 visual smoke，覆盖研究页、回测详情和交易详情。该证据只修正共享图表视觉与防回归合同，不改变 CandleProvider、data sync、回测、交易或实盘能力等级。

补充：阶段 1 K 线图表 canvas 尺寸污染自恢复在 2026-07-01 继续收紧；共享 `TradingViewChart` 会记录外部 fixed viewport 的可信 `lastSize`，并通过 `MutationObserver` / resize 流程检查 lightweight-charts 生成的内部 canvas。一旦内部 canvas 的 CSS 尺寸超过 fixed viewport、bitmap/CSS scale 低于 `0.75` 或高于 `2.25`、或 X/Y scale 偏差超过 `0.2`，组件会把 canvas 宽高恢复到上一帧健康尺寸或 bitmap 推导尺寸，并用 fixed viewport 尺寸作为上限，避免价格轴文字在同一浏览器会话里变成巨型或极小。`scripts/research-chart-height-smoke.mjs` 的污染场景已从只污染高度扩展到同时污染 canvas 宽度/高度并等待两帧恢复；组件测试覆盖人为注入 `600px x 9000px` 的内部 canvas 后，外部 fixed viewport resize 会恢复为受限宽高。该证据只修正共享 K 线图表视觉稳定性，不改变 CandleProvider、data sync、回测、交易或实盘能力等级。
