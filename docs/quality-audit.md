...
- `DataSyncHealth` 新增 `invalid` 枚举，并进入后端 OpenAPI contract 和前端 generated DTO。
...
- 研究页任务表能展示 `dataHealth=invalid` 的中文文案，并用错误态标签，而不是默认灰色标签。
- 不清洗历史行，不自动修复，不新增补同步策略，不改变 CandleProvider 查询语义。

修复范围：

- `internal/data/data_sync_model.go` 新增 `DataSyncHealthInvalid`。
- `internal/store/postgres/data_sync_task_scan.go` 在任务窗口 lateral 查询中增加 `invalid_count/has_invalid`，并把 list health CASE 接入 `invalid`。
- `internal/web/api/contract_schema.go` 和 `web/frontend/src/types/api.generated.ts` 同步 `DataSyncHealth` enum。
- `web/frontend/src/components/tables/DataSyncTaskTable.vue` 将 `invalid` 映射为 error 标签。
- `internal/store/postgres/integration_data_sync_invalid_health_test.go` 覆盖真实 PostgreSQL legacy 零价格 K 线下的任务列表 `dataHealth=invalid`。
- `web/frontend/src/components/tables/DataSyncTaskTable.test.ts` 覆盖任务表显示“数据异常”。

验证：

- `scripts/generate-api-types.sh` 通过。
- `go test ./internal/store/postgres -run '^TestIntegrationListDataSyncTasksReportsInvalidCandleHealth$' -count=1` 通过。
- `pnpm --dir web/frontend exec vitest run src/components/tables/DataSyncTaskTable.test.ts` 通过，1 个测试文件 / 12 条测试。
- `go test ./...` 通过。
- `go vet ./...` 通过。
- `pnpm --dir web/frontend run typecheck` 通过。
...
- 任务列表只暴露 `invalid` 状态，不返回异常行明细；具体 `issues` 仍在 `/api/candles` 图表窗口结果里查看。
