...
- 这不是生产级 instrument 搜索或在线校验：前端只校验格式，不证明交易所存在该标的；回测 / 交易任务表单仍有独立 symbol 输入体验，后续需要统一到同一市场数据源边界。
