...
- 不改变当前操作员 `GET /api/auth/me`、当前 session 列表、审计 / 健康页面的读取权限。
...
- 不改变当前操作员 `GET /api/auth/me`、当前 session 列表、审计 / 健康 / api-contract 页面的读取权限。
