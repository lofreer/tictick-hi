...
- 本轮只是单实例 `hi sync` 内的周期刷新；没有做跨实例分布式调度锁、精确交易所权重限流或 catalog 同步失败的系统健康告警。
