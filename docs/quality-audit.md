...
- 不新增独立的 admin 强制撤销任意用户指定 session API，不新增风险评分触发的自动强制下线。
...
- 仍缺独立 admin 指定 session 强制撤销入口、风险触发强制下线、外部告警和不可篡改审计归档。
- 审计事件仍写入主库，不提供外部不可篡改证明、签名、独立归档或告警。
