...
| 登录会话 | demo | 保留后加强 | HttpOnly session cookie、CSRF double-submit 写保护、登录失败持久化节流、当前操作员 session 列表、基础来源地址 / User-Agent 上下文、非当前 session 撤销、基础密码长度 / 复杂度 / 弱密码字典策略、当前操作员自禁用保护和最后启用操作员保护已进入 API / 系统管理边界；登录成功 / 失败、退出和会话撤销会进入基础操作审计；仍缺更完整弱密码字典 / 历史 / 轮换、RBAC / 角色级自保护规则和生产级设备上下文 |
...
- 交易所账号密钥 digest 风险已在阶段 6 切片关闭到 `demo`；历史非 AES-GCM 行标记为 `legacy`。
- live executor 仍禁用，testnet/sandbox、幂等提交、真实交易所提交和生产密钥管理仍未建立。
- 登录会话基础审计、持久化限流、基础来源上下文、基础密码长度 / 复杂度 / 弱密码字典策略、当前操作员自禁用保护和最后启用操作员保护已在后续补充关闭到 demo 边界；更完整弱密码字典 / 历史 / 轮换、RBAC / 角色级自保护规则和生产级设备上下文仍未完成。
...
- 当时操作台账号启停没有 RBAC、自保护规则或强密码策略；基础操作审计、基础密码长度 / 复杂度 / 弱密码字典策略、当前操作员自禁用保护和最后启用操作员保护已在后续阶段 7 补充中覆盖到 `demo` 边界。
...
- 这只是基础 session 管理，不是生产级登录安全；持久化限流、基础来源上下文、基础密码长度 / 复杂度 / 弱密码字典策略、当前操作员自禁用保护和最后启用操作员保护已在后续补充关闭到 demo 边界，仍缺更完整弱密码字典 / 历史 / 轮换、RBAC / 角色级自保护、设备指纹 / 变更告警和生产级审计。
...
- `data.ValidateOperatorPassword` 和 `MinOperatorPasswordLength=8` 集中定义操作员基础密码长度、复杂度和弱密码字典策略，要求密码至少包含一个字母和一个数字，并拒绝明确常见的默认弱密码。
- API 创建操作员、bootstrap operator、PostgreSQL `CreateOperator` 和 API 测试 fake repository 均复用同一校验。
- API 单测覆盖创建操作员时短密码、缺数字密码和常见弱密码返回 `400`，data 单测覆盖短密码、缺数字、缺字母和常见弱密码边界。
...
- 本补充只统一基础长度、字母/数字复杂度和有限弱密码字典策略，不能声明生产级密码安全。
...
| 登录会话 | demo | HttpOnly session、CSRF double-submit、登录失败持久化节流、session 列表 / 来源上下文和撤销有 route / smoke 覆盖；登录成功 / 失败、退出、session 撤销已进入基础操作审计；基础密码长度 / 复杂度 / 弱密码字典策略、当前操作员自禁用保护和最后启用操作员保护已覆盖 | 无更完整弱密码字典/历史/轮换/RBAC、角色级自保护规则和生产级设备上下文 |
