...
- `repairSourceTaskId` 只覆盖由 `repair-gaps` 新建的补同步任务；历史同窗口任务和图表“修复首个缺口”创建的任务仍没有源任务关系。
