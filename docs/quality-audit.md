...
- `SaveDataSyncResult` 还不能证明调用方 workerID 与持锁 worker 完全一致，后续需要接口层携带 worker identity 或统一 lease transition helper。
