...
- 本小节自身不向 worker 日志、notification provider、Binance / OKX 或其他外部系统传播 `traceparent`；worker 日志和 notification provider 传播见后续补充。
...
- traceparent 已进入 API HTTP 边界、access log、API-created task 行、data sync / backtest / trading / notify worker task 日志、notification delivery 和 provider 外发，尚未跨交易所、更广外部系统或其他子命令传播。
...
- 本小节自身不把 `traceparent` 注入 worker 业务日志、notification delivery、provider 请求、交易所请求或其他子命令；worker 日志和 notification delivery / provider 传播见后续补充。
- 不引入 OpenTelemetry SDK、span lifecycle、采样策略、外部 tracing backend、日志 sink 或 retention policy。
...
- traceparent 已保存到 API-created task 行，并在后续补充进入 data sync / backtest / trading / notify worker task 日志、notification delivery 和 provider 外发；exchange 调用和其他子命令仍未传播 W3C trace context。
- 项目整体仍为 `scaffold`，不能升级为 usable 或 production-safe。
...
- 本小节不把 `traceparent` 注入 notification delivery / notify worker 日志、notification provider 请求、交易所请求或其他子命令；notification delivery / provider traceparent 见后续补充。
- 不引入 OpenTelemetry SDK、span lifecycle、采样策略、外部 tracing backend、日志 sink 或 retention policy。
...
- trace ID 已进入 data sync / backtest / trading worker 业务日志；notification delivery / provider traceparent 已由后续补充覆盖，provider 之外的 exchange 调用和跨子命令 trace context 仍未传播。
- 项目整体仍为 `scaffold`，不能升级为 usable 或 production-safe。
...
- 不向 Binance / OKX / exchange market requests 或其他子命令传播 `traceparent`。
- 不引入 OpenTelemetry SDK、span lifecycle、采样策略、外部 tracing backend、日志 sink 或 retention policy。
...
- traceparent 已覆盖 API HTTP 边界、API-created task、trading notification、notify worker 日志和 notification provider 外发；exchange / broader external systems / 子命令间传播仍未覆盖。
- 项目整体仍为 `scaffold`，不能升级为 usable 或 production-safe。
