...
- 研究页工具栏使用紧凑控件组，symbol 输入桌面宽度收敛到 `clamp(180px, 16vw, 260px)`。
- 研究页图表槽保持固定高度预算，移动和窄桌面有独立高度/边距约束。
- `TradingViewChart` 右侧价格轴按视口收敛为 48/52/56px，减少非绘图区空白。
- 交易详情和回测详情均采用上图表、下双栏；摘要栏固定窄宽，列表栏占主要宽度。

修复范围：

- `ResearchPage.css` 收紧工具栏、symbol 输入宽度、图表槽高度和左右内边距。
- `detailChartLayout.css` 统一详情页图表高度和内边距。
...
- `scripts/quality-gate.sh` 通过。
- `docker compose up -d --build api` 通过，本地 `http://127.0.0.1:8080/readyz` 返回 `{"status":"ok"}`。
- `BASE_URL=http://127.0.0.1:8080 SMOKE_SETTLE_MS=1000 node scripts/research-chart-height-smoke.mjs` 通过：1440 桌面 chart/tv `648px`，2048 桌面 `820px`，812 窄桌面 `732px`，390 移动 `567px`，高度稳定。
- `BASE_URL=http://127.0.0.1:8080 SMOKE_SETTLE_MS=1000 node scripts/stage8-visual-smoke.mjs` 通过，桌面 / 移动、浅色 / 深色核心页面无横向溢出。
- 临时 Chrome DevTools 详情页抽样通过：回测详情 chart `612px`、下方列宽 `340px/1036px`；交易详情 chart `612px`、下方列宽 `340px/1036px`；document width `1440px`。
