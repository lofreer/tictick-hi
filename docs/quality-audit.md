...
补充：阶段 1 K 线图表坐标轴在 2026-07-01 回到正常交易终端比例；本段覆盖前述 `56px/36px`、右侧轴 `208/196/132px` 和墨迹高度 `38px/26px` 旧合同。参考 `tictickbot` / `tictick-lite` 现有 K 线图表未使用夸张轴字号的基准，共享 `TradingViewChart` 坐标轴字号回到桌面/窄桌面 `20px`、移动端 `18px`，覆盖研究页、回测详情和交易详情；右侧价格轴 minimumWidth 调整为桌面 `100px`、窄桌面 `94px`、移动端 `88px`，继续完整显示价格数值且不恢复 `k/K/m/M/b/B` 缩写；时间轴边缘 padding 回收到 `24px` 到 `48px`，避免右侧留白被时间轴 padding 放大；visual smoke 右侧价格轴最大宽度 `116px`，主图宽度占比下限校准为桌面 `0.90`、窄桌面 `0.82`、移动端 `0.70`，坐标轴 band 高度上限 `72px`，坐标轴文字墨迹高度下限为桌面/窄桌面 `14px`、移动端 `12px`。已执行 `git diff --check`、`scripts/check-research-chart-layout.sh`、`node --check scripts/stage8-visual-smoke.mjs`、`node --check scripts/research-chart-height-smoke.mjs`、`pnpm --dir web/frontend exec vitest run src/components/chart/TradingViewChart.test.ts src/pages/ResearchPage.layout.test.ts src/pages/DetailPages.layout.test.ts`、`pnpm --dir web/frontend run typecheck`、`pnpm --dir web/frontend run test`、`pnpm --dir web/frontend run build`、`go test ./...`、`go vet ./...`、`scripts/quality-gate.sh`、`docker compose up -d --build api`、`curl -fsS http://127.0.0.1:8080/readyz`、`BASE_URL=http://127.0.0.1:8080 SMOKE_SAMPLES=4 SMOKE_INTERVAL_MS=120 SMOKE_SETTLE_MS=1000 SMOKE_TOTAL_TIMEOUT_MS=180000 node scripts/research-chart-height-smoke.mjs` 和 `BASE_URL=http://127.0.0.1:8080 SMOKE_SETTLE_MS=800 SMOKE_TOTAL_TIMEOUT_MS=300000 node scripts/stage8-visual-smoke.mjs`。该证据只恢复正常图表比例，不改变 CandleProvider、data sync、回测、交易或实盘能力等级。
