...
| 登录会话 | demo | 保留后加强 | HttpOnly session cookie、CSRF double-submit 写保护、可配置登录失败持久化节流、当前操作员 session 列表、基础来源地址 / User-Agent 上下文、当前会话 IP / User-Agent 变化提示和 `auth.session_context_change` 24h 去重 warning 审计、非当前 session 撤销、当前操作员修改密码并撤销同账号其它会话、禁用操作员撤销该账号既有 session 且成功审计记录 `revokedSessionCount`、管理员可独立撤销指定 operator 的全部 session 且成功审计记录 `revokedSessionCount`、管理员可查看并精确撤销指定 operator 单个 session、管理员可重置指定 operator 密码并撤销该账号 sessions、可配置最近密码历史复用保护、基础密码长度 / 复杂度 / 扩展弱密码字典 / 不含用户名策略、当前操作员自禁用保护、最后启用操作员保护、admin/operator 基础角色、最后启用 admin 保护和当前操作员自角色变更 / 自 session 全量撤销 / 当前 session 精确撤销 / 管理页自密码重置保护已进入 API / 系统管理边界；登录成功 / 失败、退出、会话撤销、当前 session 撤销拒绝、密码修改成功 / 失败、当前操作员自禁用拒绝、当前操作员自角色变更拒绝、角色变更、operator 创建 / 启停 / 角色变更 store 冲突失败、管理员独立撤销 operator sessions 成功 / 失败 / 自撤销拒绝、管理员精确撤销 operator session 成功 / 失败、管理员重置 operator 密码成功 / 失败 / 自重置拒绝和非 admin 操作员管理拒绝会进入基础操作审计；operator 列表读取 / 管理动作、系统通知读取 / retry / channel 管理、交易所账号读取 / 创建、审计读取 / 导出 / hash chain 校验和 api-contract 读取要求 admin；仍缺密码定期轮换 / 自助忘记密码或账号恢复、MFA、细粒度 RBAC / 角色策略、多人审批、设备指纹 / 可信设备和风险评分策略 |
...
| 系统管理 / 运维健康 | demo | 保留后加强 | 操作台账号可创建、启停和变更角色，可查看指定 operator sessions、精确撤销单个 session、独立撤销全部 session、重置指定 operator 密码并撤销该账号 sessions，禁用操作员会撤销该账号既有 session 并在成功审计记录撤销数量，支持 admin/operator 基础角色，operator 列表读取 / 管理写操作、系统通知读取 / retry / channel 管理、交易所账号读取 / 创建、审计读取 / 导出 / hash chain 校验、api-contract 读取和 market instrument catalog 手动同步要求 admin，且会阻止当前操作员禁用自己、变更当前操作员角色、从管理页撤销自己的全部 session、撤销当前 session、从管理页重置自己的密码、禁用最后一个启用操作员、禁用最后一个启用 admin 或把最后一个启用 admin 降权，其中当前操作员修改密码失败、当前操作员自禁用、当前操作员自角色变更、operator 创建 / 启停 / 角色变更 store 冲突或缺失失败、operator sessions revoke 成功 / 失败 / 自撤销拒绝、operator session 精确撤销成功 / 失败、operator password reset 成功 / 失败 / 自重置拒绝、operator list 非 admin 拒绝、notification list / retry / channel 管理非 admin 拒绝、exchange account list / create 非 admin 拒绝、audit event list / page / export / hash-chain verify 非 admin 拒绝、api-contract 非 admin 拒绝、notification retry store 失败、notification channel 创建 / 更新 / 删除 / 启停 store 失败和 exchange account 创建 store 失败会写入失败审计，market instrument catalog 手动同步成功、admin 拒绝、unavailable、fetch failure 和 replace store failure 会写入基础操作审计，且 notification target / exchange account key/secret / operator password / market 外部错误详情不进入失败审计 metadata；当前操作员 session 可查看来源地址 / User-Agent、撤销非当前会话并修改密码，基础操作审计页/API 可查看登录和系统管理写操作，审计列表支持同 limit 边界的 cursor page API 和 CSV 导出，审计页可查看事件 hash / 前序 hash 短码并触发 hash chain 在线校验，新写入审计事件会返回 `previousHash` / `eventHash` 并由 PostgreSQL store 用事务级 advisory lock 串行形成 demo 级 hash chain，API 可在线校验已哈希审计事件链并报告 legacy unhashed skipped 计数，`hi audit-prune` 可 dry-run/显式执行审计保留裁剪并写入数据库本地 retention anchor，审计事件客户端上下文会 trim 并按 255 rune 截断，审计 metadata 写库前会 trim、最多保留 20 项且 key/value 分别限制为 64/255 rune，审计列表 limit 已在 API 层归一化到 1..500，运维健康页/API 展示数据库、api、worker count、heartbeat、locked_until 和 instrument catalog 同步状态；仍缺细粒度 RBAC / 角色策略、多人审批、外部不可篡改审计存储 / 签名 / 自动化归档和生产监控 |
...
- 交易所账号密钥 digest 风险已在阶段 6 切片关闭到 `demo`；历史非 AES-GCM 行标记为 `legacy`。
- live executor 仍禁用，testnet/sandbox、幂等提交、真实交易所提交和生产密钥管理仍未建立。
- 登录会话基础审计、可配置持久化限流、基础来源上下文、基础密码长度 / 复杂度 / 弱密码字典 / 不含用户名策略、当前操作员自禁用保护和最后启用操作员保护已在后续补充关闭到 demo 边界；更完整弱密码字典 / 历史 / 轮换、RBAC / 角色级自保护规则和生产级设备上下文仍未完成。
...
- `data.Repository`、PostgreSQL store 和 API 测试 fake repository 新增 `SetNotificationChannelEnabled`。
- 新增 `POST /api/system/notifications/channels/:id/enable` 和 `/disable`，返回更新后的 `NotificationChannel`。
- 通道启停写入基础操作审计：`notification_channel.enable` / `notification_channel.disable`。
- OpenAPI contract 暴露新增启停路由。

范围外：

- 通道名称 / provider / target 更新、通道删除、前端启停按钮、生产级模板、限流 / 熔断、外部回执、密钥轮换、审计签名。

验证：

- `go test ./internal/web/api ./internal/store/postgres -run 'Test(SystemNotificationChannel|NotificationChannelStoreSetEnabled|APIContract)' -count=1 -v`
...
- 本补充只补后端/API 启停能力；系统通知页前端操作、通道更新 / 删除和生产级通知治理仍未完成。
...
- `systemApi` 新增通知通道启停方法，使用后端 `/enable` / `/disable` 显式 action 路由。
- 系统通知页通道表新增启停动作列，按通道当前状态展示启用 / 停用按钮、loading 状态和成功 / 失败反馈。
- 前端 API service 测试和页面交互测试覆盖通道启停调用。

范围外：

- 通道名称 / provider / target 更新、通道删除、生产级模板、限流 / 熔断、外部回执、密钥轮换、审计签名。
...
- `scripts/quality-gate.sh`
- `git diff --check`
- 一次性 headless Chrome 检查：mock `/api/*` 后打开 `/system/notifications`，点击通道停用并确认 `/api/system/notifications/channels/nc_ops/disable` 请求。
...
- `data.Repository`、PostgreSQL store 和 API 测试 fake repository 新增 `UpdateNotificationChannel` / `DeleteNotificationChannel`。
- 新增 `PUT /api/system/notifications/channels/:id`，复用通知通道创建请求结构做完整字段替换。
- 新增 `DELETE /api/system/notifications/channels/:id`，删除成功返回 204，重复删除返回 404。
- 通道更新 / 删除写入基础操作审计：`notification_channel.update` / `notification_channel.delete`。
- OpenAPI contract 暴露新增更新 / 删除路由。

范围外：

- 系统通知页通道更新 / 删除操作、生产级模板、限流 / 熔断、外部回执、密钥轮换、审计签名。

验证：

- `go test ./internal/web/api ./internal/store/postgres -run 'Test(SystemNotificationChannel|NotificationChannelStore(UpdateDelete|SetEnabled)|APIContract|APIMethod)' -count=1 -v`
- `go test ./internal/web/api ./internal/store/postgres -count=1`
- `go test ./...`
- `go vet ./...`
- `scripts/check-file-size.sh`
...
- 全系统 worker lease、登录会话更完整弱密码字典 / 历史 / 轮换 / RBAC / 角色级自保护规则 / 更完整生产级设备上下文仍未完成。
...
- 当时操作台账号启停没有 RBAC、自保护规则或强密码策略；基础操作审计、基础密码长度 / 复杂度 / 弱密码字典 / 不含用户名策略、当前操作员自禁用保护和最后启用操作员保护已在后续阶段 7 补充中覆盖到 `demo` 边界。
...
- RBAC、角色级自保护规则、生产级会话审计、更完整弱密码字典 / 历史 / 轮换、设备指纹、IP / UA 变更告警。
...
- 这只是基础 session 管理，不是生产级登录安全；可配置持久化限流、基础来源上下文、基础密码长度 / 复杂度 / 弱密码字典 / 不含用户名策略、当前操作员自禁用保护和最后启用操作员保护已在后续补充关闭到 demo 边界，仍缺更完整弱密码字典 / 历史 / 轮换、RBAC / 角色级自保护、设备指纹 / 变更告警和生产级审计。
...
- `hi api` 通过 `AUTH_LOGIN_FAILURE_LIMIT`、`AUTH_LOGIN_FAILURE_WINDOW` 和 `AUTH_LOGIN_LOCKOUT` 配置限流次数、滑动窗口和锁定时长，默认保持 `5 / 5m / 5m`。
- `data.Repository`、PostgreSQL store 和 API 测试 fake 均提供登录限流检查、失败记录和清理方法。
- 限流触发的登录请求写入匿名 `auth.login` 失败审计，reason 为 `rate_limited`。

范围外：

- 更完整弱密码字典、MFA、RBAC、角色级自保护规则、设备指纹、IP / UA 变更告警、边缘层全局限流和生产级审计留存。

验证：

- `go test ./internal/web/api ./internal/store/postgres -count=1`
- `go test ./cmd/hi -run TestLoadAPICommandConfig -count=1`
- `go test ./...`
- `go vet ./...`
- `scripts/quality-gate.sh`
- `scripts/check-file-size.sh`
...
- 当前补充只把 API 层登录失败节流从进程内存态推进到可配置 PostgreSQL demo 边界；生产还需要更完整的账号安全策略、设备上下文和边缘/网关层防护。
...
- RBAC、角色权限、最后一个管理员角色保护、多人审批、更完整弱密码字典、MFA、不可篡改审计。
...
- 本补充只防止当前操作员误禁用自己；仍未实现角色模型、最后管理员角色保护和完整账号生命周期策略。
...
- `data.OperatorLastEnabledError` 以 `ErrInvalidState` 作为 cause，API 可继续映射为 `409 invalid_state`。
- API 测试 fake repository 和 PostgreSQL store 在禁用操作员时都会拒绝禁用最后一个已启用操作员。
- PostgreSQL store 的禁用路径在事务内按 id 顺序锁定所有已启用操作员后再更新，避免并发禁用两个账号时把启用账号数降到 0。
- 已有当前操作员自禁用保护仍在 API 层先执行；本补充覆盖 repository/store 直接调用或非当前账号禁用路径的兜底保护。

范围外：

- RBAC、角色权限、最后一个管理员角色保护、多人审批、账号恢复流程、更完整弱密码字典、MFA、不可篡改审计。

验证：

- `go test ./internal/data ./internal/web/api ./internal/store/postgres -run 'Test(OperatorLastEnabledErrorPreservesInvalidStateCause|RepositoryRejectsDisablingLastEnabledOperator|OperatorStoreRejectsDisablingLastEnabledOperator)' -count=1 -v`
- `go test ./internal/data ./internal/web/api ./internal/store/postgres -count=1`
- `go test ./...`
- `go vet ./...`
- `scripts/check-file-size.sh`
- `scripts/quality-gate.sh`
...
- 本补充只保证系统至少保留一个启用操作员；没有区分管理员/普通操作员角色，不能替代 RBAC、最后管理员角色保护或完整账号生命周期策略。
...
- `data.ValidateOperatorPassword` / `ValidateOperatorPasswordForUsername` 和 `MinOperatorPasswordLength=8` 集中定义操作员基础密码长度、复杂度、弱密码字典和不含用户名策略，要求密码至少包含一个字母和一个数字，拒绝明确常见的默认弱密码，并拒绝包含 3 个及以上字符用户名的密码；弱密码字典已在后续阶段 8 补充中扩展为规范化 key 匹配。
- API 创建操作员、bootstrap operator、PostgreSQL `CreateOperator` 和 API 测试 fake repository 均复用同一校验。
- API 单测覆盖创建操作员时短密码、缺数字密码、常见弱密码和包含用户名密码返回 `400`，data 单测覆盖短密码、缺数字、缺字母、常见弱密码和用户名包含边界。

范围外：

- 泄露密码库、密码历史、过期轮换、重置流程、MFA、SSO、账户锁定策略。

验证：

- `go test ./internal/data ./internal/web/api ./internal/store/postgres ./cmd/hi -count=1`
- `go test ./...`
- `go vet ./...`
- `scripts/check-file-size.sh`
- `scripts/quality-gate.sh`
...
- 本补充只统一基础长度、字母/数字复杂度、有限弱密码字典和不含用户名策略；弱密码字典覆盖面已在后续补充中加强，但仍不能声明生产级密码安全。
...
- `docker compose up -d --build api`
- `curl -fsS http://127.0.0.1:8080/readyz`
- 本地 HTTP smoke：登录后创建并禁用操作员，`GET /api/system/audit-events?limit=30` 返回 `auth.login`、`operator.create`、`operator.disable`，响应不包含 `secret123A`、`apiSecret`、`tokenHash`、`tictick_hi_session`。
...
| 登录会话 | demo | HttpOnly session、CSRF double-submit、可配置登录失败持久化节流、session 列表 / 来源上下文和撤销有 route / smoke 覆盖；登录成功 / 失败、退出、session 撤销已进入基础操作审计；基础密码长度 / 复杂度 / 弱密码字典 / 不含用户名策略、当前操作员自禁用保护和最后启用操作员保护已覆盖 | 无更完整弱密码字典/历史/轮换/RBAC、角色级自保护规则和生产级设备上下文 |
...
| 系统管理 / 运维健康 | demo | 操作台账号启停、当前操作员自禁用保护、最后启用操作员保护、当前操作员 session 来源上下文 / 撤销、基础操作审计页、健康页 worker 统计和通知/账号管理可用 | 无 RBAC、角色级自保护、不可篡改审计和生产监控 |
...
### 阶段 8 notification channel required text trim 约束补充
...
- API 创建 / 更新 notification channel 时，`name` 和 `target` 改为按 `strings.TrimSpace` 校验必填，拒绝纯空白值。
- 新增 `0040_notification_channel_trimmed_required_text.sql`，为 `notification_channels` 新写入 / 更新增加 `btrim(name) <> '' AND btrim(target) <> ''` CHECK。
- 新约束使用 `NOT VALID`，避免历史脏行阻塞迁移，但保护迁移后的直接 SQL 写入路径。
- PostgreSQL 约束测试拆到独立 `notification_channel_constraints_test.go`，避免继续扩大既有约束测试文件。

范围外：

- 不清洗或修复历史 notification channel 空白值。
- 不在本轮 VALIDATE 新约束，不改变 provider、outbox、worker retry 或 UI 表单交互。

当前验证：

- `go test ./internal/web/api ./internal/store/postgres -run 'TestSystemNotificationChannelUpdateValidatesRequest|TestIntegrationNotificationChannelTrimmedRequiredTextConstraint' -count=1 -v` 通过；PostgreSQL 集成用例因未设置 `TICTICK_TEST_DATABASE_URL` 按约定跳过。
- `go test ./internal/web/api ./internal/store/postgres -count=1` 通过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
- `scripts/check-file-size.sh` 通过。
...
- 未执行 PostgreSQL 真实数据库约束命中测试；当前本机未设置 `TICTICK_TEST_DATABASE_URL`。
- 未执行浏览器 / 视觉 smoke；本轮没有前端渲染变更。
...
### 阶段 8 operator username required text trim 约束补充
...
- 登录入口和系统管理创建 operator 入口改为按 `strings.TrimSpace` 校验 username / password 必填，纯空白输入返回 400。
- 新增 `0041_operator_trimmed_username.sql`，为 `operators` 新写入 / 更新增加 `btrim(username) <> ''` CHECK。
- 新约束使用 `NOT VALID`，避免历史脏行阻塞迁移，但保护迁移后的直接 SQL 写入路径。
- 单元测试覆盖登录空白 username、创建 operator 空白 username；PostgreSQL 集成测试覆盖直接 SQL 空白 username 约束。

范围外：

- 不清洗或修复历史 operator 空白 username。
- 不在本轮 VALIDATE 新约束，不改变 username 规范化、登录匹配、RBAC、MFA 或密码策略。

当前验证：

- `go test ./internal/web/api ./internal/store/postgres -run 'TestLoginRejectsBlankUsername|TestCreateOperatorRejectsBlankUsername|TestIntegrationOperatorTrimmedUsernameConstraint' -count=1 -v` 通过；PostgreSQL 集成用例因未设置 `TICTICK_TEST_DATABASE_URL` 按约定跳过。
- `go test ./internal/web/api ./internal/store/postgres -count=1` 通过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
- `scripts/check-file-size.sh` 通过。
...
- 旧数据如果已经存在纯空白 username，本轮不会自动修复，后续需要独立清洗和 VALIDATE 证据。
- 项目整体仍为 `scaffold`，不能升级为 usable 或 production-safe。

### 阶段 8 exchange account required text trim 约束补充
...
- 系统管理创建 exchange account 入口改为按 `strings.TrimSpace` 校验 exchange / alias / apiKey / apiSecret 必填，纯空白输入返回 400。
- 新增 `0042_exchange_account_trimmed_required_text.sql`，为 `exchange_accounts` 新写入 / 更新增加必填文本 `btrim(...) <> ''` CHECK。
- 新约束使用 `NOT VALID`，避免历史脏行阻塞迁移，但保护迁移后的直接 SQL 写入路径。
- 单元测试覆盖创建 exchange account 空白 exchange；PostgreSQL 集成测试覆盖直接 SQL 空白 exchange 约束。

范围外：

- 不清洗或修复历史 exchange account 空白字段。
- 不在本轮 VALIDATE 新约束，不改变 AES-GCM 凭据加密、KMS / secret manager、密钥轮换或账号启停能力。

当前验证：

- `go test ./internal/web/api ./internal/store/postgres -run 'TestSystemExchangeAccountRejectsBlankFields|TestIntegrationExchangeAccountTrimmedRequiredTextConstraint' -count=1 -v` 通过；PostgreSQL 集成用例因未设置 `TICTICK_TEST_DATABASE_URL` 按约定跳过。
- `go test ./internal/web/api ./internal/store/postgres -count=1` 通过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
- `scripts/check-file-size.sh` 通过。
...
- 旧数据如果已经存在纯空白 exchange account 字段，本轮不会自动修复，后续需要独立清洗和 VALIDATE 证据。
- 项目整体仍为 `scaffold`，不能升级为 usable 或 production-safe。

### 阶段 8 task required text trim 约束补充
...
- 数据同步任务创建入口改为按 `strings.TrimSpace` 校验 exchange / symbol / interval 必填，纯空白输入返回 400。
- 回测任务创建入口改为按 `strings.TrimSpace` 校验 name / exchange / symbol / interval / strategyId 必填，纯空白输入返回 400。
- 交易任务创建入口改为按 `strings.TrimSpace` 校验 name / type / exchange / symbol / interval / strategyId / accountId 必填，纯空白输入返回 400。
- 新增 `0043_task_trimmed_required_text.sql`，为 `data_sync_tasks`、`backtest_tasks` 和 `trading_tasks` 新写入 / 更新增加必填文本 `btrim(...) <> ''` CHECK。
- 新约束使用 `NOT VALID`，避免历史脏行阻塞迁移，但保护迁移后的直接 SQL 写入路径。
- 单元测试覆盖 data sync / backtest / trading 创建入口纯空白字段；PostgreSQL 集成测试覆盖三类任务直接 SQL 纯空白字段约束。

范围外：

- 不清洗或修复历史任务表纯空白字段。
- 不在本轮 VALIDATE 新约束，不改变任务状态机、market catalog 校验、CandleProvider、worker claim 或前端表单行为。

当前验证：

- `go test ./internal/web/api ./internal/store/postgres -run 'TestDataSyncTaskRoutesRejectBlankRequiredText|TestCreateBacktestRejectsBlankRequiredText|TestCreateTradingRejectsBlankRequiredText|TestIntegrationTaskTrimmedRequiredTextConstraints' -count=1 -v` 通过；PostgreSQL 集成用例因未设置 `TICTICK_TEST_DATABASE_URL` 按约定跳过。
...
- 旧任务数据如果已经存在纯空白必填字段，本轮不会自动修复，后续需要独立清洗和 VALIDATE 证据。
- 项目整体仍为 `scaffold`，不能升级为 usable 或 production-safe。

### 阶段 8 audit event required text trim 约束补充
...
- 新增 `0044_audit_event_trimmed_required_text.sql`，为 `audit_events` 新写入 / 更新增加 id / action / resource_type / outcome `btrim(...) <> ''` CHECK。
- 新约束使用 `NOT VALID`，避免历史脏行阻塞迁移，但保护迁移后的直接 SQL 写入路径。
- PostgreSQL 集成测试覆盖直接 SQL 空白 action 约束，并把既有 invalid outcome 测试补上明确 constraint 断言。

范围外：

- 不清洗或修复历史 audit event 空白字段。
- 不在本轮 VALIDATE 新约束，不新增审计签名、hash chain、外部审计 sink、留存策略、RBAC 或 UI 行为。

当前验证：

- `go test ./internal/store/postgres -run 'TestIntegrationAuditEventConstraintsRejectInvalidOutcome|TestIntegrationAuditEventConstraintsRejectBlankRequiredText' -count=1 -v` 通过；PostgreSQL 集成用例因未设置 `TICTICK_TEST_DATABASE_URL` 按约定跳过。
...
- 旧审计数据如果已经存在纯空白必填字段，本轮不会自动修复，后续需要独立清洗和 VALIDATE 证据。
...
### 阶段 8 market instrument sync failed error code 补充
...
- API error catalog 新增 HTTP 400 `market_instrument_sync_failed`。
- `POST /api/market/instruments/sync` 在已注册 client 但外部抓取失败时返回 `market_instrument_sync_failed`，不再使用泛化 `request_failed`。
- 失败仍按既有路径调用 `RecordMarketInstrumentSyncFailure` 写入 sync status。
- 重新生成 `web/frontend/src/types/api.generated.ts`，使前端 `APIErrorCode` union 包含新增错误码。
- 测试覆盖 fetch failure 路由响应 code、失败记录、API error catalog / schema enum。

范围外：

- 不改变外部错误 message、失败脱敏策略、重试策略、sync status schema、catalog pause / restore 行为或前端提示文案。
- 不新增真实交易所可达性探测、重试队列或分布式同步调度。

当前验证：

- `go test ./internal/web/api -run 'TestMarketInstrumentSyncRouteRecordsFetchFailure|TestAPIErrorCatalogHasUniqueKnownCodes|TestAPIErrorResponseSchemaUsesCatalogEnum' -count=1 -v` 通过。
- `scripts/generate-api-types.sh` 通过。
- `go test ./internal/web/api -count=1` 通过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
...
- 未执行真实 Binance / OKX instrument sync 外网失败 smoke；本轮不改变外部调用路径。
- 未执行浏览器 / 视觉 smoke；本轮没有前端渲染变更。
...
### 阶段 8 operator self-disable failure audit 补充
...
- 当前登录操作员调用 `POST /api/system/operators/{id}/disable` 禁用自己时，仍返回 `operator_self_disable_forbidden`。
- 自禁用被拒绝时新增 `operator.disable` 审计事件，`outcome=failure`，metadata 写入 `reason=self_disable_forbidden`、username 和 enabled。
- 测试覆盖自禁用不会改变账号状态，并且失败审计包含拒绝原因。
- 模块评级摘要同步记录当前操作员自禁用拒绝会进入基础操作审计。

范围外：

- 不改变当前操作员自禁用的 HTTP status、错误码、message、CSRF/session 行为或前端提示文案。
- 不新增 RBAC、角色级自保护、审计签名、防篡改存储、审计导出或保留策略。

当前验证：

- `go test ./internal/web/api -run 'TestOperatorCannotDisableSelf|TestSystemAuditEventsRouteRecordsSecurityActions|TestAPIErrorCatalogHasUniqueKnownCodes|TestAPIErrorResponseSchemaUsesCatalogEnum' -count=1 -v` 通过。
- `go test ./internal/web/api -count=1` 通过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
- `scripts/check-file-size.sh` 通过。
...
- 未执行浏览器 / 视觉 smoke；本轮没有前端渲染变更。
- 未执行不可篡改审计、审计保留、审计导出或合规归档测试；这些仍不在当前 scaffold 范围内。

剩余风险：

- API server 仍缺更多失败路径审计覆盖、RBAC 和生产级审计边界。
- 系统管理仍缺角色级自保护规则、不可篡改审计和生产监控。
- 项目整体仍为 `scaffold`，不能升级为 usable 或 production-safe。

### 阶段 8 current session revoke failure audit 补充
...
- 当前登录操作员调用 `DELETE /api/auth/sessions/{id}` 撤销当前 session 时，仍返回 `auth_current_session_revoke_forbidden`。
- 当前 session 撤销被拒绝时新增 `auth.session_revoke` 审计事件，`outcome=failure`，metadata 写入 `reason=current_session_revoke_forbidden`。
- 测试覆盖当前 session 撤销失败响应 code/message 不变，且失败审计包含拒绝原因。
- 模块评级摘要同步记录当前 session 撤销拒绝会进入基础操作审计。

范围外：

- 不改变非当前 session 撤销、logout 当前 session、CSRF/session cookie 行为、HTTP status、错误码、message 或前端提示文案。
- 不新增设备指纹、会话风险评分、强制下线策略、审计签名、防篡改存储、审计导出或保留策略。

当前验证：

- `go test ./internal/web/api -run 'TestAuthSessionManagementRoutes|TestSystemAuditEventsRouteRecordsSecurityActions|TestWriteStoreErrorMapsAuthCurrentSessionRevokeForbidden' -count=1 -v` 通过。
- `go test ./internal/web/api -count=1` 通过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
- `scripts/check-file-size.sh` 通过。
...
- 登录会话仍缺更完整弱密码字典 / 历史 / 轮换、RBAC、角色级自保护规则和生产级设备上下文。
- API server 仍缺更多失败路径审计覆盖、RBAC 和生产级审计边界。
- 项目整体仍为 `scaffold`，不能升级为 usable 或 production-safe。

### 阶段 8 audit event list limit clamp 补充
...
- `GET /api/system/audit-events?limit=` 在 API 层将缺失、非法、零值和负数归一化为默认 100。
- `GET /api/system/audit-events?limit=` 在 API 层将超过 500 的值归一化为 500。
- 测试覆盖默认、非法、零值、负数、有效值、上限值和超上限值。
- 模块评级摘要同步记录审计列表 limit 已在 API 层归一化到 1..500。

范围外：

- 不改变前端默认请求 limit、审计事件 schema、排序、分页模型、PostgreSQL store 内部兜底 clamp 或审计列表 UI。
- 不新增 cursor pagination、审计导出、保留策略或归档策略。

当前验证：

- `go test ./internal/web/api -run 'TestParseAuditLimitNormalizesInvalidAndOversizedValues|TestSystemAuditEventsRouteRecordsSecurityActions' -count=1 -v` 通过。
- `go test ./internal/web/api -count=1` 通过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
- `scripts/check-file-size.sh` 通过。
...
- 未执行浏览器 / 视觉 smoke；本轮没有前端渲染变更。
- 未执行 PostgreSQL 真实数据库 limit 参数集成测试；store 内部已有 `normalizeAuditEventLimit` 兜底，本轮只收敛 API handler 边界。

剩余风险：

- 审计列表 cursor page API 和 CSV 导出已在后续补充关闭到 demo 边界；仍缺保留策略、不可篡改存储和归档策略。
- 系统管理仍缺 RBAC、角色级自保护规则和生产监控。
- 项目整体仍为 `scaffold`，不能升级为 usable 或 production-safe。

### 阶段 8 audit event client context bound 补充
...
- `recordAuditEvent` 和 `recordAnonymousAuditEvent` 记录 remote addr / User-Agent 时复用 session 上下文 trim / 截断逻辑。
- 审计事件客户端上下文按 `strings.TrimSpace` 清理，并按 255 rune 截断，避免异常长请求头直接进入审计存储和列表响应。
- 测试覆盖失败登录匿名审计事件中的 `X-Forwarded-For` 首地址 trim 和超长 User-Agent 截断。
- 模块评级摘要同步记录审计事件客户端上下文会 trim 并按 255 rune 截断。

范围外：

- 不改变 session 存储字段、审计事件 schema、真实代理信任模型、IP 解析策略或前端审计列表展示。
- 不新增设备指纹、地理位置识别、风险评分、审计签名、防篡改存储或归档策略。

当前验证：

- `go test ./internal/web/api -run 'TestAuditEventClientContextIsTrimmedAndBounded|TestFailedLoginWritesAnonymousAuditEvent|TestLoginStoresSessionClientContext' -count=1 -v` 通过。
- `go test ./internal/web/api -count=1` 通过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
- `scripts/check-file-size.sh` 通过。
...
- 未执行浏览器 / 视觉 smoke；本轮没有前端渲染变更。
- 未执行真实反向代理链路验证；当前仍沿用既有 `X-Forwarded-For` 首地址策略。
- 未执行 PostgreSQL 真实数据库超长 User-Agent 写入测试；本轮 API handler 已在写入前截断。

剩余风险：

- 登录会话仍缺设备指纹、可信设备、IP / UA 变更告警、地理位置识别、session 风险评分和强制下线策略。
- 审计仍缺不可篡改存储、保留和归档策略；CSV 导出已在后续补充关闭到 demo 边界。
- 项目整体仍为 `scaffold`，不能升级为 usable 或 production-safe。

### 阶段 8 audit metadata bound 补充
...
- PostgreSQL store `RecordAuditEvent` 写入 metadata 前会复制并归一化 metadata map。
- metadata key / value 会 `strings.TrimSpace`，空 key 会跳过。
- metadata 最多保留 20 项，按 key 排序后 deterministic 保留；key 最长 64 rune，value 最长 255 rune。
- 测试覆盖 metadata key/value trim 和截断、空 key 跳过、条目数上限和排序后保留边界。
- 模块评级摘要同步记录审计 metadata 写库前的条目数和长度边界。

范围外：

- 不改变审计事件 schema、现有审计 action / resource / outcome 语义、前端审计列表展示或 PostgreSQL migration。
- 不新增审计签名、防篡改存储、导出、保留或归档策略。

当前验证：

- `go test ./internal/store/postgres -run 'TestNormalizeAuditMetadata|TestIntegrationAuditEventsRoundTrip|TestIntegrationAuditEventConstraints' -count=1 -v` 通过；PostgreSQL 集成用例因未设置 `TICTICK_TEST_DATABASE_URL` 按约定跳过。
- `go test ./internal/store/postgres -count=1` 通过；PostgreSQL 集成用例因未设置 `TICTICK_TEST_DATABASE_URL` 按约定跳过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
- `scripts/check-file-size.sh` 通过。
- `scripts/quality-gate.sh` 通过。
...
- 未执行 PostgreSQL 真实数据库 metadata 写入集成测试；当前本机未设置 `TICTICK_TEST_DATABASE_URL`。
- 未执行浏览器 / 视觉 smoke；本轮没有前端渲染变更。
...
### 阶段 8 notification outbound text bound 补充
...
- provider 外发 title 会 `strings.TrimSpace` 并限制为 200 rune。
- provider 外发 body 会 `strings.TrimSpace` 并限制为 4000 rune。
- Telegram / 飞书 / email 使用的组合通知文本会限制为 4096 rune。
- Webhook JSON payload 的 `title` / `body` 改为发送归一化后的文本，不再直接发送原始 delivery title/body。
- Email subject 改为使用归一化 title，空 title 仍回退 `tictick-hi notification`。
- 测试覆盖 webhook payload title/body 截断、通用 notification text 边界，以及既有 webhook / Telegram / 飞书 / email payload 行为。

范围外：

- 不改变 notification / outbox 数据库存储文本、交易策略生成通知内容、provider target、retry/backoff、投递状态或前端通知列表展示。
- 不新增真实第三方 provider 投递 smoke、模板系统、provider 级限流、回执解析或第三方消息 ID 持久化；provider message ID 持久化见后续补充。

当前验证：

- `go test ./internal/notification -run 'TestWebhookProvider|TestNotificationTextBoundsOutboundText|TestTelegramProviderPostsSendMessagePayload|TestFeishuProviderPostsTextMessage|TestEmailProviderBuildsSMTPMessageFromEnvironment' -count=1 -v` 通过。
- `go test ./internal/notification -count=1` 通过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
- `scripts/check-file-size.sh` 通过。
...
- 未执行真实 Telegram / 飞书 / SMTP / webhook 外网投递 smoke；本轮只覆盖本地 provider 单元测试。
- 未执行浏览器 / 视觉 smoke；本轮没有前端渲染变更。

剩余风险：

- 通知仍缺生产级模板、provider 级限流、回执解析、投递延迟指标和完整统一 worker lease；provider message ID 持久化见后续补充。
...
- 通知仍缺生产级模板、多实例 / provider 级限流、回执解析、投递延迟指标和完整统一 worker lease；provider message ID 持久化见后续补充。
...
### 阶段 8 audit event hash chain 补充
...
- `0045_audit_event_hash_chain.sql` 为 `audit_events` 增加 `previous_hash` / `event_hash` 字段、64 字符 hex 格式 CHECK、非空 event hash 唯一索引和非空 previous hash 唯一索引。
- PostgreSQL store 写入 audit event 时在事务内获取 `pg_advisory_xact_lock`，读取当前未被任何 `previous_hash` 引用的非空链尾 `event_hash`，再用 SHA-256 计算当前事件 hash，避免并发写入产生同一 previous hash 分叉。
- hash payload 覆盖 previous hash、事件 ID、操作者、动作、资源、结果、请求上下文、归一化 metadata 和微秒精度 `created_at`。
- `AuditEvent` API contract 和生成的 `web/frontend/src/types/api.generated.ts` 新增可选 `previousHash` / `eventHash` 字段。
- 单元测试覆盖 hash 计算确定性、previous hash 链接和 metadata 变化；PostgreSQL 集成测试在有 `TICTICK_TEST_DATABASE_URL` 时覆盖 round-trip hash 链接。

范围外：

- 不回填历史 `audit_events` hash；迁移后的第一条新 hash 事件会从空 previous hash 开始。
- 不新增外部 append-only 审计 sink、HSM / KMS 签名、密钥轮换、归档留存策略、审计导出或 UI 展示。
- 不把审计写入和业务写操作合并为同一事务。

当前验证：

- `go test ./internal/store/postgres -run 'TestAuditEventHashIsDeterministicAndChainsPreviousHash|TestNormalizeAuditMetadata|TestIntegrationAuditEventsRoundTrip|TestIntegrationAuditEventConstraints' -count=1 -v` 通过；PostgreSQL 集成用例因未设置 `TICTICK_TEST_DATABASE_URL` 按约定跳过。
- `go test ./internal/data ./internal/web/api -run 'TestFrontendAPIResponseTypesMatchContractFields|TestFrontendAPIGeneratedTypesAreCurrent|TestSystemAuditEventsRouteRecordsSecurityActions|TestAPIContractValidatesWithExternalOpenAPIValidator' -count=1 -v` 通过。
- `scripts/generate-api-types.sh` 通过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
- `scripts/check-api-contract-drift.sh` 通过。
...
- 未执行 PostgreSQL 真实数据库 hash chain round-trip / 约束命中测试；当前本机未设置 `TICTICK_TEST_DATABASE_URL`。
- 未执行 Docker Compose migration audit / smoke；本轮尚未启动 Compose 数据库。
- 未执行浏览器 / 视觉 smoke；本轮没有页面布局或渲染变更。

剩余风险：

- hash chain 只覆盖迁移后的新写入事件，历史审计行没有回填且未 VALIDATE。
- 数据库管理员仍可同时篡改数据和 hash，仍缺外部不可篡改存储、签名、保留和归档。
- 审计写入和业务写操作仍不是同一事务，部分成功 / 审计失败边界仍不是 production-safe。
- 项目整体仍为 `scaffold`，不能升级为 usable 或 production-safe。

### 阶段 8 notification provider duration 可观察性补充
...
- `0046_notification_delivery_duration.sql` 为 `notifications` 和 `notification_outbox` 增加 `last_delivery_duration_ms`，并用 `NOT VALID` 非负 CHECK 保护迁移后的新写入 / 更新。
- `notification.Runner` 只测量实际 `provider.Deliver` 调用耗时，不把本地 provider throttle 等待计入 provider duration。
- delivery 成功和失败都会把 provider 调用耗时传给 repository；retry 重新排队和通道禁用失败会清空该字段，避免 pending 记录展示旧尝试耗时。
- `Notification` API contract 和生成的 `web/frontend/src/types/api.generated.ts` 新增可选 `lastDeliveryDurationMs` 字段。
- notify worker 成功 / 失败日志新增 `provider`、`channel`、`attempt_count`、`max_attempts` 和 `delivery_duration_ms` 字段。

范围外：

- 不新增 Prometheus / OpenTelemetry metrics exporter，不新增告警规则或 SLO。
- 本补充不读取第三方 provider response 里的 message ID、rate-limit header 或 receipt 状态；provider message ID 持久化见后续补充。
- 不改变 notification provider 协议、outbox claim、retry/backoff、lease 或前端展示。

当前验证：

- `go test ./internal/notification -run 'TestRunnerRecordsProviderDeliveryDuration|TestRunnerDeliversLocalNotification|TestRunnerSchedulesRetry|TestRunnerAppliesProviderMinIntervalBetweenDeliveries' -count=1 -v` 通过。
- `scripts/generate-api-types.sh` 通过。
- `go test ./internal/notification ./internal/store/postgres ./internal/web/api -run 'TestRunnerRecordsProviderDeliveryDuration|TestRunnerDeliversLocalNotification|TestFrontendAPIResponseTypesMatchContractFields|TestFrontendAPIGeneratedTypesAreCurrent|TestAPIContractValidatesWithExternalOpenAPIValidator|TestIntegration' -count=1 -v` 通过；PostgreSQL 集成用例因未设置 `TICTICK_TEST_DATABASE_URL` 按约定跳过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
- `scripts/check-api-contract-drift.sh` 通过。
- `pnpm --dir web/frontend run typecheck` 通过。
...
- 未执行 PostgreSQL 真实数据库 duration 字段 round-trip / 约束命中测试；当前本机未设置 `TICTICK_TEST_DATABASE_URL`。
- 未执行真实 Telegram / 飞书 / SMTP / webhook 外网投递 duration smoke。
- 未执行 Prometheus / OpenTelemetry 指标采集或告警验证；本轮只提供持久化字段和结构化日志。
- 未执行浏览器 / 视觉 smoke；本轮没有页面布局或渲染变更。

剩余风险：

- `lastDeliveryDurationMs` 是最近一次 provider 调用耗时，不是完整队列等待时间、端到端通知延迟或第三方确认延迟。
- 通知仍缺生产级模板、多实例 / provider 级限流、回执解析、指标 exporter、告警和完整统一 worker lease；provider message ID 持久化见后续补充。
- 项目整体仍为 `scaffold`，不能升级为 usable 或 production-safe。

### 阶段 8 notification provider message ID 可观察性补充
...
- `0047_notification_provider_message_ids.sql` 为 `notifications` 和 `notification_outbox` 增加 `provider_message_id`，并用 `NOT VALID` 长度 CHECK 保护迁移后的新写入 / 更新。
- `notification.Provider` 返回 `NotificationDeliveryResult`，成功投递可携带 `ProviderMessageID`。
- webhook / Telegram / 飞书 provider 会从成功响应 JSON 的常见 `messageId` / `message_id` / `id` 字段，以及 `result` / `data` 包裹对象中解析 provider message ID；local / webhook-demo / email 保持空值。
- `notification.Runner` 在成功投递时把 provider message ID 传给 repository；失败、retry 重新排队和通道禁用失败会清空该字段，避免 pending / failed 记录展示旧外部 ID。
- notify worker 成功日志会在 provider message ID 非空时输出归一化后的 `provider_message_id` 字段，便于日志与第三方后台互查。
- `Notification` API contract 和生成的 `web/frontend/src/types/api.generated.ts` 新增可选 `providerMessageId` 字段，系统通知页和交易详情通知列表会展示该值。

范围外：

- 不新增第三方回执轮询、状态 callback、阅读/送达确认或 webhook inbound receipt 解析。
- 不读取 provider rate-limit header，不建立 provider 配额模型。
- 不宣称所有第三方 provider 都会返回稳定、可唯一索引或可用于回执查询的 message ID。

当前验证：

- `go test ./internal/notification` 通过。
- `go test ./internal/notification ./internal/store/postgres ./internal/web/api` 通过；PostgreSQL 集成用例因未设置 `TICTICK_TEST_DATABASE_URL` 按约定跳过。
- `scripts/generate-api-types.sh` 通过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
- `scripts/check-api-contract-drift.sh` 通过。
- `pnpm --dir web/frontend run typecheck` 通过。
- `pnpm --dir web/frontend run test` 通过。
- `pnpm --dir web/frontend exec vitest run src/pages/SystemNotificationsPage.test.ts src/pages/SystemNotificationsPage.layout.test.ts src/pages/DetailPages.layout.test.ts` 通过。
- `scripts/check-file-size.sh` 通过。
- `scripts/quality-gate.sh` 通过。
- `git diff --check` 通过。
...
- 未执行 PostgreSQL 真实数据库 provider message ID 字段 round-trip / 约束命中测试；当前本机未设置 `TICTICK_TEST_DATABASE_URL`。
- 未执行真实 Telegram / 飞书 / SMTP / webhook 外网投递 smoke；本轮只覆盖本地 HTTP mock 和单元测试。
- 未执行浏览器 / 视觉 smoke；本轮只是紧凑文本字段展示。

剩余风险：

- `providerMessageId` 是 provider 成功响应中解析出的关联 ID，不代表第三方已送达、已读或已确认。
- 不同 provider 的 ID 字段语义不统一，当前不做跨 provider 唯一约束或回执查询。
- 通知仍缺生产级模板、多实例 / provider 级限流、回执解析、指标 exporter、告警和完整统一 worker lease。
- 项目整体仍为 `scaffold`，不能升级为 usable 或 production-safe。

### 阶段 8 audit event CSV export 补充
...
- 新增 `GET /api/system/audit-events/export`，复用审计列表 `limit` 归一化边界，返回最近审计事件 CSV。
- CSV 响应设置 `Content-Type: text/csv; charset=utf-8`、`Content-Disposition` 附件文件名和 `X-Content-Type-Options: nosniff`。
- CSV 字段覆盖事件 ID、时间、操作者、动作、资源、结果、请求上下文、`previousHash` / `eventHash` 和 JSON metadata。
- CSV 单元格对 `= + - @` 公式前缀做中性化，降低被表格软件打开时的公式注入风险。
- OpenAPI contract 支持声明非 JSON 成功响应，导出接口声明 `text/csv`。
- 系统操作审计页标题区新增同源 CSV 下载按钮。

范围外：

- 不新增 cursor pagination、过滤条件、后台异步导出任务、外部归档存储或留存策略。
- 不新增外部 append-only 审计 sink、签名、KMS/HSM 或不可篡改存储。
- 不改变审计写入事务边界、hash chain 语义或现有审计列表 JSON API。

当前验证：

- `go test ./internal/web/api -run 'TestSystemAuditEvents|TestAuditEventsCSV|TestParseAuditLimit|TestAPIContractCoversCurrentFrontendRoutes|TestAPIContractDeclaresAuditEventsCSVExport|TestAPIMethodNotAllowedContracts|TestAPIContractValidatesWithExternalOpenAPIValidator' -count=1` 通过。
- `go test ./internal/web/api` 通过。
- `scripts/generate-api-types.sh` 通过。
- `scripts/check-api-contract-drift.sh` 通过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
- `pnpm --dir web/frontend run typecheck` 通过。
- `pnpm --dir web/frontend run test` 通过。
- `pnpm --dir web/frontend exec vitest run src/pages/SystemAuditEventsPage.layout.test.ts` 通过。
- `scripts/check-file-size.sh` 通过。
- `scripts/quality-gate.sh` 通过。
- `git diff --check` 通过。
...
- 未执行 PostgreSQL 真实数据库审计导出 round-trip；导出复用现有 `ListAuditEvents` repository 边界。
- 未执行浏览器 / 视觉 smoke；本轮只新增标题区下载按钮并由类型检查和静态 layout test 锁定。

剩余风险：

- CSV 导出只覆盖最近 `limit` 条审计事件，不替代 cursor pagination、全量归档或合规留存。
- 导出文件仍来自主库可读数据，不提供外部不可篡改证明、签名或独立归档链路。
- 系统管理仍缺 RBAC、角色级自保护规则和生产监控。
- 项目整体仍为 `scaffold`，不能升级为 usable 或 production-safe。

### 阶段 8 audit event cursor pagination 补充
...
- 保留 `GET /api/system/audit-events` 数组响应兼容性，新增 `GET /api/system/audit-events/page` 返回 `{ events, nextCursor }`。
- `nextCursor` 使用审计事件 `createdAt + id` 编码为 opaque cursor，API 解析非法 cursor 时返回 400。
- PostgreSQL store page 查询按 `created_at DESC, id DESC` 稳定排序，并用 `(created_at, id) < (cursorCreatedAt, cursorID)` 翻页。
- page 查询多取 1 条判断是否存在下一页，并只把当前页最后一条事件编码为 `nextCursor`。
- 系统操作审计页改用 page API 首屏加载，并提供“加载更早审计”按钮追加旧事件。
- OpenAPI contract 新增 `AuditEventPage` schema、page route、`cursor` query 参数和 400 响应。

范围外：

- 不改变旧审计数组 API，不改变 CSV 导出语义。
- 不新增服务端过滤、全文搜索、虚拟滚动、后台异步导出或全量归档。
- 不新增审计留存策略、外部不可篡改存储或签名。

当前验证：

- `go test ./internal/data ./internal/store/postgres ./internal/web/api -run 'TestAuditEventCursor|TestSystemAuditEvents|TestParseAuditLimit|TestAPIContractCoversCurrentFrontendRoutes|TestAPIContractDeclaresAuditEvents|TestAPIMethodNotAllowedContracts|TestFrontendAPIGeneratedTypesAreCurrent|TestAPIContractValidatesWithExternalOpenAPIValidator' -count=1` 通过。
- `scripts/generate-api-types.sh` 通过。
- `scripts/check-api-contract-drift.sh` 通过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
- `pnpm --dir web/frontend run typecheck` 通过。
- `pnpm --dir web/frontend run test` 通过。
- `pnpm --dir web/frontend exec vitest run src/services/api/system.test.ts src/pages/SystemAuditEventsPage.layout.test.ts` 通过。
- `scripts/check-file-size.sh` 通过。
- `scripts/quality-gate.sh` 通过。
- `git diff --check` 通过。
- 前端 dev server HTTP smoke：`curl -fsS http://127.0.0.1:5173/system/audit-events` 返回 Vite HTML。

未执行：

- 未执行 PostgreSQL 真实数据库 cursor page round-trip；当前本机未设置 `TICTICK_TEST_DATABASE_URL`。
- 未完成 in-app browser 视觉验证；Browser 工具运行脚本失败于 `sandboxCwd must be an absolute file URI`，本轮已用 dev server HTTP smoke、typecheck 和 layout test 兜底。

剩余风险：

- cursor page 只解决最近审计事件的人工翻页，不替代长期归档、合规留存或搜索能力。
- page API 仍读取主库审计表，不提供外部不可篡改证明或独立签名。
- 系统管理仍缺 RBAC、角色级自保护规则和生产监控。
- 项目整体仍为 `scaffold`，不能升级为 usable 或 production-safe。

### 阶段 8 audit event hash UI 可观察性补充
...
- 系统操作审计页新增 hash 列，展示 `eventHash` 前 12 位短码。
- 当 `previousHash` 存在时，同列展示前序 hash 前 12 位短码，便于人工确认链路相邻关系。
- 静态 layout test 锁定 hash 列、短 hash helper 和中英文文案 key。

范围外：

- 不在 UI 展示完整 64 字符 hash；完整值仍通过 API/CSV 获取。
- 不新增 hash chain 在线校验、签名、外部 append-only sink 或不可篡改存储。
- 不回填历史审计事件 hash。
...
- 未完成 in-app browser 视觉验证；Browser 工具运行脚本失败于 `sandboxCwd must be an absolute file URI`，本轮用 typecheck 和 layout test 兜底。

剩余风险：

- UI 短码只用于快速识别，不足以替代完整 hash 校验。
- hash chain 仍来自主库可读数据，不提供外部不可篡改证明、签名或独立归档链路。
- 项目整体仍为 `scaffold`，不能升级为 usable 或 production-safe。

### 阶段 8 audit event hash chain verify API 补充
...
- 新增 `GET /api/system/audit-events/hash-chain/verify`，只读校验已有 `event_hash` 的审计事件链。
- 校验会重新 canonicalize metadata 后计算事件 hash，检测 hash mismatch、missing previous hash、fork、cycle、disconnected chain、多 root 和多 tail。
- 结果返回 `status`、`checkedCount`、`skippedCount`、root/tail count、broken event id、message 和 checkedAt。
- 历史未回填 `event_hash` 的审计事件不伪装成已覆盖，计入 `skippedCount` 并让结果进入 warning。
- OpenAPI contract 新增 `AuditEventHashChainVerification` schema 和 verify route。

范围外：

- 不新增外部 append-only sink、KMS/HSM 签名、独立归档、留存策略或自动告警。
- 不回填历史 `audit_events` hash，不 VALIDATE 旧数据。
- 不改变审计写入事务边界，也不把 hash 校验结果接入运维健康状态。

当前验证：

- `go test ./internal/store/postgres ./internal/web/api -run 'TestVerifyAuditEventHashRecords|TestSystemAuditEventsHashChainVerifyRoute|TestAPIContractDeclaresAuditEventHashChainVerification|TestAPIContractCoversCurrentFrontendRoutes|TestAPIMethodNotAllowedContracts|TestAPIContractValidatesWithExternalOpenAPIValidator' -count=1` 通过。
- `scripts/generate-api-types.sh` 通过。
- `scripts/check-api-contract-drift.sh` 通过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
...
- 未执行 PostgreSQL 真实数据库 verify route round-trip；当前本机未设置 `TICTICK_TEST_DATABASE_URL`。
- 未执行外部篡改演练、告警联动或归档校验。

剩余风险：

- verify API 只能发现主库内 hash 链不一致，无法阻止同时篡改数据和 hash 的数据库管理员。
- 仍缺外部不可篡改存储、签名、留存、归档和告警。
- 项目整体仍为 `scaffold`，不能升级为 usable 或 production-safe。

### 阶段 8 audit event hash chain verify UI 补充
...
- 系统操作审计页新增“校验哈希”按钮，调用 `GET /api/system/audit-events/hash-chain/verify`。
- 页面展示校验 status、服务端 message、checked count 和 skipped count。
- service 单测覆盖 verify API 路径，layout test 锁定按钮、结果 alert 和中英文文案 key。

范围外：

- 不把校验结果接入系统健康状态、告警或自动阻断。
- 不新增外部不可篡改存储、签名、归档或留存策略。
...
- 页面入口只是人工校验，不提供持续监控、告警或外部证明。
- 项目整体仍为 `scaffold`，不能升级为 usable 或 production-safe。

### 阶段 8 audit event retention prune CLI 补充
...
- 新增 `hi audit-prune` 显式维护命令；默认 dry-run，只有传入 `--execute` 才会删除审计事件。
- `hi audit-prune` 要求 `--retention-days` 或 `AUDIT_RETENTION_DAYS` 为正整数，非法配置会在打开 PostgreSQL 前失败且不回显敏感值。
- PostgreSQL store 新增审计保留裁剪流程：事务内获取 audit hash chain advisory lock，先校验当前链，再只裁剪 cutoff 前的安全 hash chain 前缀和 cutoff 前 legacy unhashed 事件。
- 实际裁剪 hashed 审计事件前会写入数据库本地 `audit_event_retention_anchors`，裁剪后再次校验保留链再提交。
- hash chain verify 现在接受 retention anchor 指向的缺失前序 hash，避免合法保留裁剪后误报 broken chain。
- Go 子命令手册和生产运行手册记录 dry-run、执行、锚点语义和外部归档边界。

范围外：

- 不新增外部 append-only 审计存储、KMS/HSM 签名、自动归档调度、归档完整性校验或告警。
- 不在 API / 前端暴露 prune 操作，不把 prune 接入系统健康状态。
- 不回填历史 unhashed 审计事件，也不自动删除所有早于 cutoff 但不属于当前安全 hash chain 前缀的 hashed 事件。

当前验证：

- `go test ./cmd/hi -run 'TestLoadAuditPruneCommandConfig|TestSafeConfigSummary' -count=1` 通过。
- `go test ./internal/store/postgres -run 'TestPlanAuditEventHashPrune|TestVerifyAuditEventHashRecords' -count=1` 通过。
- `scripts/stage8-command-config-smoke.sh` 通过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
- `scripts/check-file-size.sh` 通过。
...
- 未执行真实 PostgreSQL prune round-trip；当前本机未设置 `TICTICK_TEST_DATABASE_URL`。
- 未执行外部归档、签名、恢复或告警联动演练。

剩余风险：

- retention anchor 仍在主库内，无法防止数据库管理员同时篡改审计事件、hash 和 anchor。
- 审计保留仍需人工或部署层调度；外部不可篡改归档和监控未关闭。
- 项目整体仍为 `scaffold`，不能升级为 usable 或 production-safe。

### 阶段 8 operator role RBAC 补充
...
- `operators` 新增 `role` 字段，支持 `admin` / `operator`；历史和 bootstrap 操作员默认 `admin`，新建操作员不传 role 时默认 `operator`。
- API contract / generated TypeScript DTO 暴露 `Operator.role`，`CreateOperator.role` 为可选字段。
- 系统操作员管理写操作要求当前登录操作员为 `admin`；非 admin 请求返回 403 `forbidden`，并写入失败审计事件 `reason=admin_required`。
- 禁用操作员时同时保留“至少一个启用操作员”和“至少一个启用 admin”两条保护。
- 操作员页面展示角色，并在创建操作员时提供角色选择，默认创建普通操作员。

范围外：

- 不新增角色编辑 / 降权 / 升权 API，不实现资源级权限矩阵、多人审批、MFA、SSO 或 break-glass 恢复流程。
- 不把非 operator 管理的所有业务写操作纳入细粒度 RBAC；当前只收敛系统操作员管理边界。
- 不回填历史审计事件中的 role metadata。

当前验证：

- `go test ./internal/data ./internal/store/postgres ./internal/web/api -run 'TestOperator|TestCreateOperator|TestRepositoryRejectsDisablingLast|TestWriteStoreErrorMapsOperator|TestAPIErrorCatalogHasUniqueKnownCodes|TestFrontendAPI|TestAPIContract|TestAPIMethod' -count=1` 通过。
- `scripts/generate-api-types.sh` 通过。
- `scripts/check-api-contract-drift.sh` 通过。
- `pnpm --dir web/frontend run typecheck` 通过。
- `pnpm --dir web/frontend exec vitest run src/pages/SystemOperatorsPage.test.ts src/services/api/system.test.ts` 通过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
- `pnpm --dir web/frontend run test` 通过。
- `scripts/check-file-size.sh` 通过。
- `scripts/quality-gate.sh` 通过。
- `git diff --check` 通过。
- Vite HTTP smoke：`curl -fsS http://127.0.0.1:5173/system/operators` 返回 SPA HTML。

未执行：

- 未完成 in-app Browser 视觉验证；Browser 工具仍失败于 `sandboxCwd must be an absolute file URI`，本轮用 typecheck、vitest 和 Vite HTTP smoke 兜底。
- 未执行真实 PostgreSQL operator role migration round-trip；当前本机未设置 `TICTICK_TEST_DATABASE_URL`。

剩余风险：

- 当前是粗粒度 admin/operator RBAC，不是完整权限矩阵。
- 仍缺角色变更审计/审批、MFA、账号恢复流程和生产级设备上下文。
- 项目整体仍为 `scaffold`，不能升级为 usable 或 production-safe。

### 阶段 8 operator role change 补充
...
- 新增 `POST /api/system/operators/{id}/role`，请求体为 `UpdateOperatorRole{role}`，响应仍为 `Operator`。
- 角色变更写操作要求当前登录操作员为 `admin`；非 admin 请求返回 403 `forbidden`，并写入失败审计事件 `reason=admin_required`。
- API 层拒绝当前操作员变更自己的角色，返回 409 `operator_self_role_change_forbidden`，并写入失败审计事件 `reason=self_role_change_forbidden`。
- PostgreSQL store 在把启用 admin 降权为 operator 前锁定启用操作员集合，拒绝把最后一个启用 admin 降权，复用 409 `operator_last_admin_required`。
- 成功角色变更审计写入 `previousRole`、新 `role`、`username` 和 `enabled` metadata。
- 操作员页面新增角色编辑入口；当前登录操作员的角色编辑入口在 UI 层禁用。
- API contract / generated TypeScript DTO 暴露 `UpdateOperatorRole` 和角色变更 route。

范围外：

- 不实现资源级权限矩阵、角色策略 DSL、多人审批、MFA、SSO、break-glass 恢复流程或密码轮换。
- 不新增角色删除、角色自定义、批量角色变更、角色历史查询或独立审批队列。
- 不把所有业务写操作纳入细粒度 RBAC；当前仍只收敛系统操作员管理边界。

当前验证：

- `go test ./internal/web/api -run 'TestOperator|TestUpdateOperator|TestRepositoryRejectsDemoting|TestAPIContract|TestAPIMethod|TestFrontendAPI|TestAPIErrorCatalog' -count=1` 通过。
- `go test ./internal/store/postgres -run 'TestOperatorStore|TestIntegrationOperatorTrimmedUsernameConstraint' -count=1` 通过。
- `scripts/generate-api-types.sh` 通过。
- `scripts/check-api-contract-drift.sh` 通过。
- `pnpm --dir web/frontend run typecheck` 通过。
- `pnpm --dir web/frontend exec vitest run src/pages/SystemOperatorsPage.test.ts src/services/api/system.test.ts` 通过。
...
- 未完成 in-app Browser 视觉验证；Browser 工具仍失败于 `sandboxCwd must be an absolute file URI`，本轮用 typecheck、vitest 和 Vite HTTP smoke 兜底。
- 未执行真实 PostgreSQL role update round-trip；当前本机未设置 `TICTICK_TEST_DATABASE_URL`。
- 未执行多人审批、账号恢复、MFA 或设备上下文演练。

剩余风险：

- 当前仍是粗粒度 admin/operator RBAC，不是完整权限矩阵或角色策略系统。
- 角色变更已审计但没有多人审批、独立审批队列或 break-glass 恢复流程。
- 仍缺 MFA、账号恢复流程、生产级设备上下文和外部不可篡改审计边界。
- 项目整体仍为 `scaffold`，不能升级为 usable 或 production-safe。

### 阶段 8 operator action store failure audit 补充
...
- `POST /api/system/operators/{id}/enable|disable` 在 repository 返回领域错误、not found 或 invalid state 时，会先写入 `operator.enable|disable` 失败审计，再返回原错误响应。
- `POST /api/system/operators/{id}/role` 在 repository 返回领域错误、not found 或 invalid state 时，会先写入 `operator.role` 失败审计，再返回原错误响应。
- 失败审计 metadata 使用规范化 `reason`：领域错误码优先，其次为 `not_found`、`invalid_state`，兜底为 `store_error`。
- 启停失败审计记录目标 `enabled` 值，角色变更失败审计记录 `requestedRole`。

范围外：

- 不新增审计重试队列、外部审计 sink、签名、不可篡改存储、告警或审批流。
- 不改变 operator 自禁用、自角色变更和非 admin 拒绝的既有失败审计语义。
- 不扩大到所有系统管理写操作或跨领域失败路径；当前只覆盖 operator 启停 / 角色变更 store error 分支。

当前验证：

- `go test ./internal/web/api -run 'TestOperator|TestUpdateOperator|TestRepositoryRejects|TestAPIErrorCatalog' -count=1` 通过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
- `scripts/check-file-size.sh` 通过。
- `scripts/quality-gate.sh` 通过。
...
- 未执行真实 PostgreSQL API round-trip；当前本机未设置 `TICTICK_TEST_DATABASE_URL`。

剩余风险：

- operator 之外的系统管理写操作仍可能存在未专门审计的失败分支。
- 审计事件仍写入主库，不提供外部不可篡改证明、签名、独立归档或告警。
- 项目整体仍为 `scaffold`，不能升级为 usable 或 production-safe。

### 阶段 8 notification channel store failure audit 补充
...
- `POST /api/system/notifications/channels` 在 repository 创建失败时，会先写入 `notification_channel.create` 失败审计，再返回原 500 安全错误响应。
- `PUT /api/system/notifications/channels/{id}` 在 repository 更新失败时，会先写入 `notification_channel.update` 失败审计，再返回原错误响应。
- `DELETE /api/system/notifications/channels/{id}` 在 repository 删除失败时，会先写入 `notification_channel.delete` 失败审计，再返回原错误响应。
- `POST /api/system/notifications/channels/{id}/enable|disable` 在 repository 启停失败时，会先写入对应 `notification_channel.enable|disable` 失败审计，再返回原错误响应。
- 失败审计 metadata 使用 `reason` 记录规范化 store failure；创建 / 更新失败记录 `name`、`provider`、`enabled`，启停失败记录目标 `enabled`，继续不记录 channel `target`。

范围外：

- 不新增审计重试队列、外部审计 sink、签名、不可篡改存储、告警或审批流。
- 不改变 notification retry、exchange account、operator create validation 或其它系统管理写操作失败路径。
- 不记录 notification channel target，避免把 webhook token、SMTP 凭据引用或 chat id 细节写入审计 metadata。

当前验证：

- `go test ./internal/web/api -run 'TestSystemNotificationChannel|TestOperator' -count=1` 通过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
- `scripts/check-file-size.sh` 通过。
- `scripts/quality-gate.sh` 通过。
...
- notification retry、exchange account 和其它系统管理写操作仍可能存在未专门审计的失败分支。
- 审计事件仍写入主库，不提供外部不可篡改证明、签名、独立归档或告警。
- 项目整体仍为 `scaffold`，不能升级为 usable 或 production-safe。

### 阶段 8 exchange account store failure audit 补充
...
- `POST /api/system/exchange-accounts` 在 repository 创建失败时，会先写入 `exchange_account.create` 失败审计，再返回原 500 安全错误响应。
- 失败审计 metadata 使用 `reason` 记录规范化 store failure，并记录 `exchange`、`alias` 和 `enabled`。
- 失败审计 metadata 不记录 `apiKey`、`apiSecret` 或任何密钥明文。

范围外：

- 不新增交易所账号更新 / 删除 / 启停 API，不改变凭据加密、KMS、secret manager、轮换或历史账号迁移策略。
- 不新增审计重试队列、外部审计 sink、签名、不可篡改存储、告警或审批流。
- 不扩大到 notification retry 或其它系统管理失败路径。

当前验证：

- `go test ./internal/web/api -run 'TestSystemExchangeAccount|TestAPIErrorCatalog' -count=1` 通过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
- `scripts/check-file-size.sh` 通过。
- `scripts/quality-gate.sh` 通过。
...
- 交易所账号仍缺生产级 KMS / secret manager、密钥轮换和历史账号迁移策略。
- notification retry 和其它系统管理写操作仍可能存在未专门审计的失败分支。
- 审计事件仍写入主库，不提供外部不可篡改证明、签名、独立归档或告警。
- 项目整体仍为 `scaffold`，不能升级为 usable 或 production-safe。

### 阶段 8 notification retry store failure audit 补充
...
- `POST /api/system/notifications/{id}/retry` 在 repository retry 失败时，会先写入 `notification.retry` 失败审计，再返回原错误响应。
- 失败审计的 `resourceId` 使用请求中的 notification id。
- 失败审计 metadata 使用 `reason` 记录规范化 store failure。

范围外：

- 不改变 notification retry 成功路径、outbox 状态流转、provider 路由解析或 disabled channel 处理语义。
- 不新增审计重试队列、外部审计 sink、签名、不可篡改存储、告警或审批流。
- 不扩大到其它系统管理失败路径。

当前验证：

- `go test ./internal/web/api -run 'TestSystemNotification|TestAPIErrorCatalog' -count=1` 通过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
- `scripts/check-file-size.sh` 通过。
- `scripts/quality-gate.sh` 通过。
...
- 其它系统管理写操作仍可能存在未专门审计的失败分支。
- 审计事件仍写入主库，不提供外部不可篡改证明、签名、独立归档或告警。
- 项目整体仍为 `scaffold`，不能升级为 usable 或 production-safe。

### 阶段 8 operator create store failure audit 补充
...
- `POST /api/system/operators` 在 repository 创建失败时，会先写入 `operator.create` 失败审计，再返回原 500 安全错误响应。
- 失败审计 metadata 使用 `reason` 记录规范化 store failure，并记录 `username`、规范化 `role` 和 `enabled`。
- 失败审计 metadata 不记录 `password` 或密码明文。

范围外：

- 不改变 operator 创建成功路径、密码策略、非 admin 拒绝、角色更新或启停语义。
- 不新增审计重试队列、外部审计 sink、签名、不可篡改存储、告警或审批流。
- 不扩大到所有非系统业务写操作。

当前验证：

- `go test ./internal/web/api -run 'TestCreateOperatorStoreFailureAudited|TestOperator.*Audited|TestAPIErrorCatalog' -count=1` 通过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
- `scripts/check-file-size.sh` 通过。
- `scripts/quality-gate.sh` 通过。
...
### 阶段 8 market instrument sync audit 补充
...
- `POST /api/market/instruments/sync` 成功同步后写入 `market_instrument.sync` 成功审计，resourceId 使用 exchange。
- 请求 exchange 没有已注册 client 时写入失败审计，`reason=market_instrument_sync_unavailable`。
- 外部 instrument fetch 失败且 sync status 记录成功时写入失败审计，`reason=market_instrument_sync_failed`，不把外部错误详情写入 audit metadata。
- catalog replace 写库失败时保留既有 sync failure status 记录，并写入失败审计，`reason` 使用规范化 store failure。
- 成功审计 metadata 记录 active / inactive / paused / restored 计数。

范围外：

- 不改变真实 Binance / OKX fetch、sync status schema、catalog pause / restore、错误响应 message 或前端提示文案。
- 不新增 admin-only 限制、重试队列、外部审计 sink、签名、不可篡改存储、告警或分布式调度。

当前验证：

- `go test ./internal/web/api -run 'TestMarketInstrumentSyncRouteAudits|TestMarketInstrumentSyncRouteRecordsFetchFailure|TestMarketInstrumentSyncRouteRejectsUnavailableClient|TestAPIErrorCatalog' -count=1` 通过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
- `scripts/check-file-size.sh` 通过。
- `scripts/quality-gate.sh` 通过。
...
### 阶段 8 operator list admin RBAC 补充
...
- `GET /api/system/operators` 要求当前操作员为 `admin`；非 admin 返回 403 `forbidden`。
- 非 admin operator 列表读取请求写入 `operator.list` 失败审计，metadata 包含 `reason=admin_required` 和 `actorRole`。
- 系统菜单只对 admin 显示操作员管理入口。
- 操作员管理页只对 admin 显示创建按钮和行级角色 / 启停操作。

范围外：

- 不改变当前操作员 `GET /api/auth/me`、当前 session 列表和健康页面的读取权限。
- 不新增资源级权限矩阵、角色策略 DSL、多人审批、MFA、SSO、break-glass 恢复流程或密码轮换。

当前验证：

- `go test ./internal/web/api -run 'TestOperatorManagementRequiresAdminRole|TestAPIErrorCatalog' -count=1` 通过。
- `pnpm --dir web/frontend exec vitest run src/components/layout/SystemMenu.test.ts src/pages/SystemOperatorsPage.test.ts` 通过。
- `pnpm --dir web/frontend run typecheck` 通过。
- `pnpm --dir web/frontend run test` 通过。
- `pnpm --dir web/frontend run build` 通过。
- `go test ./...` 通过。
...
- 未执行 in-app Browser 视觉验证；Browser 工具仍失败于 `sandboxCwd must be an absolute file URI: relative URL without a base`，本轮用组件测试、typecheck、build 和 Vite HTTP smoke 兜底。
- 未执行多人审批、账号恢复、MFA、设备上下文或不可篡改审计归档演练。

剩余风险：

- 当前仍是粗粒度 admin/operator RBAC，不是完整权限矩阵或角色策略系统。
- 操作员管理仍没有多人审批、独立审批队列或 break-glass 恢复流程。
- 仍缺 MFA、账号恢复流程、生产级设备上下文和外部不可篡改审计边界。

### 阶段 8 系统敏感配置 admin RBAC 补充
...
- `GET /api/system/notifications`、`POST /api/system/notifications/:id/retry`、notification channel 列表 / 创建 / 更新 / 删除 / 启停要求当前操作员为 `admin`；非 admin 返回 403 `forbidden`。
- `GET /api/system/exchange-accounts` 和 `POST /api/system/exchange-accounts` 要求当前操作员为 `admin`；非 admin 返回 403 `forbidden`。
- 非 admin 拒绝写入 `notification.list`、`notification.retry`、`notification_channel.*`、`exchange_account.list` 和 `exchange_account.create` 失败审计，metadata 包含 `reason=admin_required` 和 `actorRole`。
- 系统菜单只对 admin 显示通知管理、交易所账号管理和操作台账号管理入口。
- 通知页只对 admin 显示创建通道、通知 retry 和 channel 编辑 / 启停 / 删除操作；交易所账号页只对 admin 显示创建账号入口。
- 概览页在非 admin operator 下跳过 admin-only 系统通知读取，并隐藏通知汇总卡和投递深度指标，避免首页因 403 降级为整体加载失败。

范围外：

- 不改变当前操作员 `GET /api/auth/me`、当前 session 列表和健康页面的读取权限。
- 不新增资源级权限矩阵、角色策略 DSL、多人审批、MFA、SSO、break-glass 恢复流程、密码轮换或外部审计 sink。

当前验证：

- `go test ./internal/web/api -run 'TestSystemNotificationManagementRequiresAdminRole|TestSystemExchangeAccountManagementRequiresAdminRole|TestSystemNotificationChannelsAcceptExternalProviders|TestSystemNotificationChannelEnableDisable|TestSystemExchangeAccountRejectsBlankFields' -count=1` 通过。
- `pnpm --dir web/frontend exec vitest run src/components/layout/SystemMenu.test.ts src/pages/SystemNotificationsPage.test.ts src/pages/SystemExchangeAccountsPage.test.ts src/composables/useOverviewWorkspace.test.ts` 通过。
- `pnpm --dir web/frontend run typecheck` 通过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
- `pnpm --dir web/frontend run test` 通过。
...
- `scripts/check-scaffold-markers.sh && scripts/check-future-risk-markers.sh` 通过。
- `scripts/quality-gate.sh` 通过。
- Vite HTTP smoke：`/`、`/system/notifications`、`/system/exchange-accounts`、`/system/operators` 均返回 SPA HTML。
...
- 当前仍是粗粒度 admin/operator RBAC，不是完整权限矩阵或角色策略系统。
- 健康读取权限仍未拆成细粒度权限。
- 仍缺 MFA、多人审批、账号恢复流程、生产级设备上下文和外部不可篡改审计边界。

### 阶段 8 audit events admin RBAC 补充
...
- `GET /api/system/audit-events`、`GET /api/system/audit-events/page`、`GET /api/system/audit-events/export` 和 `GET /api/system/audit-events/hash-chain/verify` 要求当前操作员为 `admin`；非 admin 返回 403 `forbidden`。
- 非 admin 拒绝写入 `audit_event.list`、`audit_event.page`、`audit_event.export` 和 `audit_event.hash_chain_verify` 失败审计，metadata 包含 `reason=admin_required` 和 `actorRole`。
- 系统菜单只对 admin 显示操作审计入口。
- 审计页直达时只对 admin 显示 hash chain 校验、CSV 导出和刷新 header 操作。

范围外：

- 不改变当前操作员 `GET /api/auth/me`、当前 session 列表和健康页面的读取权限。
- 不新增资源级权限矩阵、角色策略 DSL、多人审批、外部 append-only 审计 sink、KMS/HSM 签名、不可篡改存储、自动归档或告警。

当前验证：

- `go test ./internal/web/api -run 'TestSystemAuditEventsRequireAdminRole|TestSystemAuditEventsRouteRecordsSecurityActions|TestSystemAuditEventsPageReturnsNextCursor|TestSystemAuditEventsHashChainVerifyRoute|TestSystemAuditEventsExportReturnsCSV' -count=1` 通过。
- `pnpm --dir web/frontend exec vitest run src/components/layout/SystemMenu.test.ts src/pages/SystemAuditEventsPage.layout.test.ts` 通过。
- `pnpm --dir web/frontend run typecheck` 通过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
- `pnpm --dir web/frontend run test` 通过。
...
- `scripts/check-scaffold-markers.sh && scripts/check-future-risk-markers.sh` 通过。
- `scripts/quality-gate.sh` 通过。
- Vite HTTP smoke：`/system/audit-events`、`/system/health`、`/system/sessions` 均返回 SPA HTML。
...
- 健康读取权限仍未拆成细粒度权限。
- 审计事件仍写入主库，不提供外部不可篡改证明、签名、独立归档或告警。
- 仍缺 MFA、多人审批、账号恢复流程和生产级设备上下文。

### 阶段 8 api-contract admin RBAC 补充
...
- `GET /api/system/api-contract` 要求当前操作员为 `admin`；非 admin 返回 403 `forbidden`。
- 非 admin 拒绝写入 `api_contract.read` 失败审计，metadata 包含 `reason=admin_required` 和 `actorRole`。

范围外：

- 不改变当前操作员 `GET /api/auth/me`、当前 session 列表和健康页面的读取权限。
- 不新增资源级权限矩阵、角色策略 DSL、多人审批、MFA、SSO、break-glass 恢复流程、OpenAPI 发布流程或外部审计 sink。

当前验证：

- `go test ./internal/web/api -run 'TestAPIContractRouteExposesOpenAPIContract|TestAPIContractRouteRequiresAdminRole|TestAPIContractCoversCurrentFrontendRoutes|TestAPIErrorCatalog' -count=1` 通过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
- `scripts/check-file-size.sh` 通过。
- `git diff --check` 通过。
...
- 未执行前端测试 / build 和 Browser 视觉验证；本切片只修改后端 RBAC 与文档，没有改动前端 UI。
...
- 新增 `POST /api/auth/password`，要求当前登录 session 和 CSRF，通过当前密码和新密码修改当前操作员密码。
- 新密码复用现有基础密码策略：长度、字母 / 数字复杂度、有限弱密码字典和不包含当前 username。
- 修改密码在 PostgreSQL store 内以事务完成：校验当前 bcrypt hash、写入新 bcrypt hash、保留当前会话并撤销同账号其它会话。
- 系统登录会话页新增修改密码表单，成功后清空输入并刷新会话列表。
- 密码修改成功 / 失败写入 `auth.password_change` 审计；metadata 只记录 `reason` 或 `revokedSessionCount`，不记录密码明文、hash 或 session token hash。
- OpenAPI contract、生成 TypeScript DTO、前端 auth service 和 contract drift 测试纳入 `ChangeOperatorPasswordRequest` / `ChangeOperatorPasswordResult`。

范围外：

- 不新增密码重置、忘记密码、密码历史、强制定期轮换、泄露密码库查询、MFA、SSO、可信设备、设备指纹、IP / UA 变更告警或风险评分。
- 不改变 admin 创建其它 operator 初始密码的能力，不新增由 admin 代改他人密码的流程。
- 不改变当前操作员 `GET /api/auth/me`、当前 session 列表和健康页面的读取权限。

当前验证：

- `go test ./internal/web/api -run 'TestChangeOperatorPassword|TestAuthSessionManagementRoutes|TestAPIContractDeclaresChangeOperatorPassword|TestAPIContractCoversCurrentFrontendRoutes|TestAPIMethodNotAllowedContracts|TestFrontendAPI(RequestTypesMatchContractSchemas|ResponseTypesMatchContractFields|AppTypesReferenceGeneratedContract|GeneratedTypesAreCurrent)' -count=1` 通过。
- `go test ./internal/store/postgres -run 'TestOperatorStoreChangesPasswordAndRevokesOtherSessions' -count=1` 通过。
- `pnpm --dir web/frontend exec vitest run src/services/api/auth.test.ts src/pages/SystemSessionsPage.test.ts` 通过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
- `pnpm --dir web/frontend run typecheck` 通过。
- `pnpm --dir web/frontend run test` 通过。
...
- `scripts/check-scaffold-markers.sh && scripts/check-future-risk-markers.sh` 通过。
- `scripts/quality-gate.sh` 通过。
- Vite HTTP smoke：`/system/sessions` 和 `/` 均返回 SPA HTML。
...
- 当前仍是基础密码策略，不覆盖泄露密码库、可配置历史深度、定期轮换、重置流程或账号恢复。
- 当前仍缺 MFA、可信设备、设备指纹、IP / UA 变更告警和生产级设备上下文。
- 审计事件仍写入主库，不提供外部不可篡改证明、签名、独立归档或告警。
...
- 新增 `0050_operator_password_history.sql`，保存 operator 历史密码 bcrypt hash，并按 `operator_id, created_at DESC, id DESC` 建索引。
- `ChangeOperatorPassword` 在同一事务内比较新密码与当前 hash、最近 5 条历史 hash；命中时拒绝修改。
- 成功修改密码时把旧 hash 写入历史，并裁剪为最近 5 条历史记录；历史深度可配置见下一小节。
- 新增 `operator_password_reused` 领域 / API 错误码，HTTP 映射为 400，OpenAPI `APIErrorCode` 和生成 TypeScript enum 同步更新。
- 密码历史复用拒绝写入 `auth.password_change` 失败审计，metadata reason 为 `password_history`，不记录密码明文、hash 或 session token hash。

范围外：

- 不回填历史密码 hash；历史保护从本迁移后的密码修改开始生效。
- 不新增密码过期轮换、密码重置 / 忘记密码、泄露密码库查询、MFA、SSO、可信设备或账号恢复流程。
- 不改变 admin 创建其它 operator 初始密码的能力，不新增由 admin 代改他人密码的流程。

当前验证：

- `go test ./internal/data -run 'TestOperatorPasswordReusedErrorPreservesInvalidStateCause|TestValidateOperatorPassword' -count=1` 通过。
- `go test ./internal/web/api -run 'TestChangeOperatorPasswordRejectsRecentPasswordReuse|TestChangeOperatorPasswordUpdatesPasswordAndRevokesOtherSessions|TestWriteStoreErrorMapsOperatorPasswordReused|TestAPIErrorCatalogHasUniqueKnownCodes|TestAPIErrorResponseSchemaUsesCatalogEnum|TestFrontendAPIGeneratedTypesAreCurrent' -count=1` 通过。
- `go test ./internal/store/postgres -run 'TestOperatorStore(ChangesPasswordAndRevokesOtherSessions|RejectsRecentPasswordReuse|PrunesPasswordHistory)' -count=1` 通过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
- `pnpm --dir web/frontend run typecheck` 通过。
- `pnpm --dir web/frontend run test` 通过。
...
- 未执行 in-app Browser 视觉验证；本切片未改变 UI 结构，只更新后端、迁移、API 错误码与生成类型。
...
- 新增 `AUTH_PASSWORD_HISTORY_LIMIT` API env，默认 `5`，允许 `0` 关闭历史密码检查；当前密码原样复用仍始终拒绝。
- `hi api` 启动前严格解析 `AUTH_PASSWORD_HISTORY_LIMIT`，非法负值会在打开 PostgreSQL 前失败，错误只包含 env 名。
- PostgreSQL store 通过运行时配置控制历史 hash 查询、写入和裁剪深度；深度为 `0` 时不读取也不写入 `operator_password_history`。
- `.env.example`、`docker-compose.yml` 和 `docs/go-command-runbook.md` 已同步新配置。
- `scripts/stage8-command-config-smoke.sh` 已覆盖非法负值，防止配置错误漏过启动前校验。

范围外：

- 不新增按角色 / operator 的细粒度密码策略，也不新增 UI 配置入口。
- 不回填历史密码 hash，不改变已有迁移边界。
- 不新增密码过期轮换、重置流程、泄露密码库查询、MFA、SSO、可信设备或账号恢复流程。

当前验证：

- `go test ./cmd/hi -run 'TestLoadAPICommandConfig(LoadsLoginRateLimitOptions|AllowsDisabledPasswordHistory|RejectsInvalidPasswordHistoryLimit)|TestSafeConfigSummaryRedactsSensitiveKeys' -count=1` 通过。
- `go test ./internal/store/postgres -run 'TestOperatorStore(RejectsRecentPasswordReuse|PrunesPasswordHistory|UsesConfiguredPasswordHistoryLimit|CanDisablePasswordHistory)' -count=1` 通过。
- `go test ./internal/web/api -run 'TestChangeOperatorPasswordRejectsRecentPasswordReuse|TestChangeOperatorPasswordUpdatesPasswordAndRevokesOtherSessions' -count=1` 通过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
- `bash -n scripts/stage8-command-config-smoke.sh && scripts/stage8-command-config-smoke.sh` 通过。
- `set -a; source .env.example; set +a; docker compose config >/tmp/tictick-hi-compose-config.yml` 通过。
- `scripts/check-file-size.sh` 通过。
- `git diff --check` 通过。
- `scripts/check-scaffold-markers.sh && scripts/check-future-risk-markers.sh` 通过。
- `scripts/quality-gate.sh` 通过。
...
- 未执行 in-app Browser 视觉验证；本切片没有前端 UI 渲染变更。
...
- `data.ValidateOperatorPassword` 的常见弱密码检查从少量精确字符串升级为规范化 key 匹配。
- 规范化会统一大小写、忽略空白 / 分隔符 / 标点，并处理 `@ -> a`、`$ -> s`、`0 -> o` 这类常见简单变体。
- 字典新增 admin / administrator / demo / localadmin / operator / ops / root / welcome / product year 等常见默认组合，继续复用原有错误消息 `password is too common`。
- API 创建 operator、bootstrap operator、当前操作员改密和 PostgreSQL store 仍复用同一个 data 层密码策略。

范围外：

- 不接入外部泄露密码库或在线密码风险评分。
- 不新增密码长度 / 复杂度配置，不改变现有最小长度 8 和字母 + 数字规则。
- 不新增密码过期轮换、重置流程、MFA、SSO、可信设备或账号恢复流程。

当前验证：

- `go test ./internal/data -run 'TestValidateOperatorPassword|TestValidateOperatorPasswordForUsername' -count=1` 通过。
- `go test ./internal/web/api -run 'TestCreateOperatorRejectsWeakPassword|TestChangeOperatorPasswordRejectsWeakNewPassword' -count=1` 通过。
- `go test ./cmd/hi -run 'TestBootstrap|TestLoadAPICommandConfig' -count=1` 通过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
- `scripts/check-file-size.sh` 通过。
- `git diff --check` 通过。
...
- 不阻断请求、不自动撤销 session，也不新增设备指纹、可信设备、地理位置、风险评分或强制下线策略；warning 审计见下一小节。
...
- 当前仍缺 MFA、可信设备、设备指纹、地理位置识别、session 风险评分和强制下线策略。
- 审计事件仍写入主库，不提供外部不可篡改证明、签名、独立归档或告警。

### 阶段 8 当前会话上下文变化 warning 审计补充
...
- 新增 `0051_audit_event_warning_outcome.sql`，允许 audit event `outcome=warning`，保留既有 `success` / `failure`。
- 当前 session 来源地址或 User-Agent 与登录时上下文不一致时，`GET /api/auth/sessions` 会记录 `auth.session_context_change` warning 审计事件，resource 为当前 `operator_session`。
- warning metadata 只包含 `remoteAddrChanged` / `userAgentChanged` 布尔值；请求当前 remote addr / User-Agent 仍走审计基础字段，不把登录时旧上下文复制到 metadata。
- 审计列表将 `warning` outcome 显示为 warning 标签，并补中英文文案。

范围外：

- 不阻断请求、不自动撤销 session、不做去重 / 限流、不新增外部告警 sink。
- 不新增设备指纹、可信设备、地理位置、风险评分、强制下线或 MFA。

当前验证：

- `go test ./internal/web/api -run 'TestLoginStoresSessionClientContext|TestSystemAuditEventsRouteRecordsSecurityActions' -count=1` 通过。
- `go test ./internal/store/postgres -run 'TestIntegrationAuditEventsRoundTrip|TestIntegrationAuditEventConstraintsRejectInvalidOutcome' -count=1` 通过。
- `pnpm --dir web/frontend exec vitest run src/pages/SystemAuditEventsPage.layout.test.ts` 通过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
- `pnpm --dir web/frontend run typecheck` 通过。
...
- 未完成 in-app Browser 视觉验证；Browser 工具仍失败于 `sandboxCwd must be an absolute file URI: relative URL without a base`，本轮用组件测试、typecheck、build 和 Vite HTTP smoke 兜底。
- 未执行外部审计 sink、告警联动、不可篡改归档或真实设备风险演练。

剩余风险：

- 同一变化在 24h 冷却后或被最近 100 条审计窗口挤出后再次打开会话页仍可能重复产生 warning。
- 仍缺设备指纹、可信设备、地理位置、风险评分、风险触发强制下线、MFA 和外部告警。
- 审计事件仍写入主库，不提供外部不可篡改证明、签名、独立归档或告警。

### 阶段 8 当前会话上下文变化 warning 审计去重补充
...
- `GET /api/auth/sessions` 记录 `auth.session_context_change` 前会读取最近 100 条 audit event。
- 同一当前 session、同一 `remoteAddrChanged` / `userAgentChanged` 组合在 24 小时内已有 warning 时，不再追加重复审计事件。
- 去重只影响审计写入；API response 仍返回 `remoteAddrChanged` / `userAgentChanged`，系统会话页仍可显示当前漂移提示。
- 审计读取失败时仍保留写入 warning，避免因去重查询异常丢掉安全信号。

范围外：

- 不做跨 session 去重，不合并不同变化组合，不改变审计 hash chain、保留策略或外部告警。
- 不新增设备指纹、可信设备、地理位置、风险评分、风险触发强制下线或 MFA。
...
- 未执行前端测试 / build 和 Browser 视觉验证；本切片只改变后端去重逻辑和文档，没有改动前端 UI。
- 未执行真实 PostgreSQL 去重并发演练；当前用 API handler 单元测试覆盖重复请求不重复写入。

剩余风险：

- 冷却基于最近 100 条审计事件和应用时钟；高频审计流量或时钟异常仍可能让重复 warning 再次写入。
- 仍缺设备指纹、可信设备、地理位置、风险评分、风险触发强制下线、MFA 和外部告警。
- 审计事件仍写入主库，不提供外部不可篡改证明、签名、独立归档或告警。

### 阶段 8 禁用操作员撤销既有 session 补充
...
- `SetOperatorEnabled(..., enabled=false)` 在禁用操作员的同一 PostgreSQL 事务内删除该 operator 的 `operator_sessions`。
- fake repository 同步清理目标 operator sessions，保证 API 单测与真实 store 行为一致。
- 管理员禁用已登录 operator 后，该 operator 的旧 session 立即无法继续访问 `/api/auth/me`。
- `operator.disable` 成功审计 metadata 会记录 handler 预读到的 `revokedSessionCount`，用于说明本次禁用前可见的有效 session 数。
- 既有最后启用操作员 / 最后启用 admin / 当前操作员自禁用保护保持不变；被这些保护拒绝的禁用请求不会清理 session。

范围外：

- 不新增按单个 session id 的 admin 强制撤销 API，不新增风险评分触发的自动强制下线。
- 不改变当前操作员修改密码时“保留当前 session、撤销同账号其它 session”的语义。
- 不新增 MFA、可信设备、设备指纹、地理位置或外部告警。

当前验证：

- `go test ./internal/web/api -run 'TestDisablingOperatorRevokesSessions|TestOperatorEnableDisableStoreFailureAudited|TestSystemAuditEventsRouteRecordsSecurityActions' -count=1` 通过。
- `go test ./internal/store/postgres -run 'TestOperatorStoreDisablingOperatorRevokesSessions|TestOperatorStoreRejectsDisablingLastEnabledOperator|TestOperatorStoreRejectsDisablingLastEnabledAdmin' -count=1` 通过。

未执行：

- 未执行前端测试 / build 和 Browser 视觉验证；本切片只改变后端 store/API 行为和文档，没有改动前端 UI。
- 未执行真实多管理员并发禁用 / 会话访问竞争演练；当前由 store 事务路径和 targeted 测试覆盖基础一致性。

剩余风险：

- 这只是账号禁用时的会话清理，不是基于设备指纹、风险评分、地理位置或 MFA 的生产级 session 风控。
- `revokedSessionCount` 是禁用前 handler 预读的审计信息；真正撤销结果以 store 事务删除为准。
- 仍缺 admin 按单个 session id 精确撤销入口、风险触发强制下线、外部告警和不可篡改审计归档。
- 审计事件仍写入主库，不提供外部不可篡改证明、签名、独立归档或告警。

### 阶段 8 管理员独立撤销 operator sessions 补充
...
- 新增 `POST /api/system/operators/{id}/sessions/revoke`，要求 admin 和 CSRF，用于撤销指定 operator 的全部 session。
- 当前登录操作员不能通过该端点撤销自己的全部 session；API 返回 `409 operator_self_session_revoke_forbidden`，并保留当前 session。
- PostgreSQL store 新增按 `operator_id` 删除 sessions 的 repository 入口并返回实际删除数量；fake repository 同步实现，便于 API 单测覆盖。
- `operator.sessions_revoke` 成功审计 metadata 记录目标 username、role、enabled 和 `revokedSessionCount`；目标缺失、store 失败、自撤销拒绝和非 admin 拒绝进入失败审计。
- OpenAPI contract、API error catalog、生成 TypeScript DTO、前端 `systemApi`、操作员管理页按钮和中英文文案同步更新。
- 操作员管理页仅 admin 可见操作列；当前 operator 行禁用撤销 sessions 按钮，其它 operator 行可直接触发撤销并刷新列表。

范围外：

- 全量撤销端点本身不承担按单个 session id 精确撤销；精确撤销见下一小节。
- 不改变当前操作员在 `/system/sessions` 撤销非当前 session、修改密码撤销其它 sessions、禁用 operator 撤销 sessions 的既有语义。
- 不新增设备指纹、可信设备、地理位置、风险评分、自动风险强制下线、MFA 或外部告警。

当前验证：

- `go test ./internal/web/api -run 'TestAdminRevokesOperatorSessions|TestOperatorCannotRevokeOwnSessions|TestOperatorManagementRequiresAdminRole|TestAPIContractDeclaresAdminSystemSchemas|TestAPIContractDeclaresAllCurrentRoutes|TestAPIErrorCatalogIncludesKnownCodes|TestFrontendAPIResponseTypesMatchContractFields|TestFrontendAPIAppTypesReferenceGeneratedContract|TestAPIMethodNotAllowedContracts' -count=1` 通过。
- `go test ./internal/store/postgres -run 'TestOperatorStoreDeletesOperatorSessionsByOperatorID|TestOperatorStoreDisablingOperatorRevokesSessions' -count=1` 通过。
- `pnpm --dir web/frontend exec vitest run src/pages/SystemOperatorsPage.test.ts src/services/api/system.test.ts` 通过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
- `pnpm --dir web/frontend run typecheck` 通过。
...
- `scripts/check-scaffold-markers.sh && scripts/check-future-risk-markers.sh` 通过。
- `scripts/quality-gate.sh` 通过。
- Vite HTTP smoke：`/` 和 `/system/operators` 均返回非空 SPA HTML。

未执行：

- 未完成 in-app Browser 视觉验证；Browser 工具仍失败于 `sandboxCwd must be an absolute file URI: relative URL without a base`，本轮用组件测试、typecheck、build 和 Vite HTTP smoke 兜底。
- 未执行真实多管理员并发撤销 / 会话访问竞争演练；当前由 repository 删除结果、API handler 单测和 PostgreSQL targeted 测试覆盖基础一致性。

剩余风险：

- 独立撤销入口是 demo 级人工管理动作，不是基于设备指纹、可信设备、地理位置或风险评分的生产级 session 风控。
- 全量撤销入口仍是批量动作；需要选择性保留远端 session 时应使用单 session 管理入口。
- 审计事件仍写入主库，不提供外部不可篡改证明、签名、独立归档或告警。

### 阶段 8 管理员精确查看 / 撤销 operator session 补充
...
- 新增 `GET /api/system/operators/{id}/sessions`，要求 admin，用于查看指定 operator 的活动 sessions。
- 新增 `DELETE /api/system/operators/{id}/sessions/{sessionId}`，要求 admin 和 CSRF，用于精确撤销指定 operator 的单个 session。
- 删除复用现有 `DeleteOperatorSessionByID` 语义；当前登录 session 仍返回 `409 auth_current_session_revoke_forbidden`，不会被系统管理路径绕过。
- `operator.session_revoke` 成功审计 metadata 记录目标 operator id、username、role、enabled；目标缺失、session 缺失、当前 session 拒绝、store 失败和非 admin 拒绝进入失败审计。
- OpenAPI contract 新增两个系统管理 session 路由，并用唯一 operationId 避免与 `/api/auth/sessions` 合同冲突。
- 操作员管理页新增 sessions 弹窗；admin 可查看目标 operator session id、来源地址、User-Agent、创建和过期时间，并撤销单个非当前 session。

范围外：

- 不新增设备指纹、可信设备、地理位置、风险评分、自动风险强制下线、MFA 或外部告警。
- 不改变当前操作员 `/system/sessions` 自助查看 / 撤销非当前 session / 修改密码撤销其它 sessions 的语义。
- 不改变全量撤销全部 sessions 的批量管理入口；精确撤销和全量撤销保持两个显式操作。

当前验证：

- `go test ./internal/web/api -run 'TestAdminListsOperatorSessions|TestAdminRevokesSingleOperatorSession|TestOperatorCannotRevokeCurrentSessionThroughAdminRoute|TestOperatorManagementRequiresAdminRole|TestAPIContractDeclaresAllCurrentRoutes|TestAPIMethodNotAllowedContracts' -count=1` 通过。
- `pnpm --dir web/frontend exec vitest run src/pages/SystemOperatorsPage.test.ts src/services/api/system.test.ts` 通过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
- `pnpm --dir web/frontend run typecheck` 通过。
...
- 未完成 in-app Browser 视觉验证；Browser 工具仍失败于 `sandboxCwd must be an absolute file URI: relative URL without a base`，本轮用组件测试、typecheck、build 和 Vite HTTP smoke 兜底。
- 未执行真实多管理员并发精确撤销 / 会话访问竞争演练；当前由 handler 单测、store 删除语义和全量 API 测试覆盖基础一致性。

剩余风险：

- 该能力仍是 demo 级人工管理动作，不是基于设备指纹、可信设备、地理位置或风险评分的生产级 session 风控。
- 管理员 session 查看和撤销仍缺多人审批、外部告警和生产级审计归档。
- 审计事件仍写入主库，不提供外部不可篡改证明、签名或独立归档。

### 阶段 8 管理员重置 operator 密码补充
...
- 新增 `POST /api/system/operators/{id}/password`，要求 admin 和 CSRF，用于管理员重置指定 operator 密码。
- 请求 / 响应合同新增 `ResetOperatorPasswordRequest{newPassword}` 和 `ResetOperatorPasswordResult{status, revokedSessionCount}`，并由 OpenAPI contract 生成前端 DTO。
- 重置复用现有 operator 密码长度 / 复杂度 / 弱密码字典 / 不含用户名策略和可配置历史复用保护；密码策略失败返回 400，历史复用返回 `operator_password_reused`。
- 当前登录 operator 不能通过管理页重置自己的密码，返回 `409 operator_self_password_reset_forbidden`，避免绕过当前操作员改密流程。
- PostgreSQL store 在事务内锁定目标 operator、写入新密码 hash、记录旧 hash 到 password history、按配置裁剪历史，并删除目标 operator 全部 sessions。
- fake repository 同步实现密码策略、历史复用和 session 撤销语义，保证 handler 单测和真实 store 行为一致。
- 审计新增 `operator.password_reset` 成功 / 失败记录；metadata 包含目标 operator 和 `revokedSessionCount`，不记录明文密码或 password hash。
- 操作员管理页新增重置密码操作和弹窗，当前 operator 自身的重置按钮置灰，成功提示撤销 session 数量。

范围外：

- 不新增自助忘记密码、邮件 / 短信重置 token、账号恢复流程、MFA、SSO、可信设备或外部风险评分。
- 不新增多人审批、外部告警、不可篡改审计存储或独立审计归档。
- 不改变当前操作员 `/system/sessions` 修改自己密码并撤销其它 sessions 的自助流程。

当前验证：

- `go test ./internal/web/api -run 'TestAdminResetsOperatorPasswordAndRevokesSessions|TestOperatorCannotResetOwnPasswordThroughAdminRoute|TestAdminResetOperatorPasswordRejectsPolicyAndHistoryFailures|TestOperatorPasswordResetRequiresAdminRole|TestAPIContractDeclaresAllCurrentRoutes|TestAPIMethodNotAllowedContracts|TestAPIErrorCatalogIncludesKnownCodes|TestFrontendAPIRequestTypesMatchContractSchemas|TestFrontendAPIResponseTypesMatchContractFields|TestFrontendAPIAppTypesReferenceGeneratedContract' -count=1` 通过。
- `go test ./internal/store/postgres -run 'TestOperatorStoreResetsOperatorPasswordAndRevokesSessions' -count=1` 通过。
- `pnpm --dir web/frontend exec vitest run src/pages/SystemOperatorsPage.test.ts src/services/api/system.test.ts` 通过。
- `go test ./...` 通过。
- `go vet ./...` 通过。
- `pnpm --dir web/frontend run typecheck` 通过。
