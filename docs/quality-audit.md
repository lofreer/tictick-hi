...
- 已有 `scripts/stage1-candle-provider-perf-smoke.sh` 覆盖 240000 根基础 `1m` 到 1000 根 `4h` 的 PostgreSQL 查询边界；单测覆盖默认 `1d limit=1000` 从 1440000 根基础 `1m` 流式聚合；仍缺长期/并发性能压测、超过 1440000 根基础 K 线的缓存/分段策略和更多异常数据边界；闭合周期信号已有 runner 侧基础过滤，未闭合 K 线不再进入策略输入。
...
| CandleProvider | demo | native/aggregated/gap/coverage/pagination/window metadata、opaque adjacent-window cursor、最多 1440000 根基础 `1m` 的有界流式聚合读取、240000 根 PostgreSQL 性能 smoke、runner 健康门禁和集成测试已覆盖 | 长期/并发性能压测、超过 1440000 根基础 K 线的缓存/分段策略、异常数据修复策略不足 |
...
- 研究页图表工具栏改为交易所、交易对、刷新、周期、窗口控制的紧凑工作条；新增图表刷新 icon 按钮。
- 研究页、交易详情、回测详情共享 `klineChartLayout.css` 固定图表槽，plot 高度收敛为桌面 `clamp(620px, 62vh, 760px)`、窄桌面 `640px`、移动端 `520px`。
...
- `ResearchPage.layout.test.ts`、`DetailPages.layout.test.ts`、`TradingViewChart.test.ts`、`check-research-chart-layout.sh`、`research-chart-height-smoke.mjs` 和 `stage8-visual-smoke.mjs` 同步为新的布局 contract。
...
- `scripts/research-chart-height-smoke.mjs` 通过：`1440x900` 图表 `620px`、`2048x1152` 图表 `714px`、`812x1320` 图表 `640px`、`390x844` 图表 `520px`，污染内部高度后 document/panel/body/chart/tv 高度均稳定。
...
- 首次 `scripts/research-chart-height-smoke.mjs` 在移动端失败：`.research-chart-panel` scrollWidth 被状态 tag 区域撑到 `1000px`。已将移动端 `.research-meta` 限制回父容器并复跑通过。
