...
- `TradingViewChart` 右侧价格轴按视口收敛为 44/48/52px，减少非绘图区空白。
...
- `scripts/research-chart-height-smoke.mjs` 固定 light theme 采样，避免继承上一次浏览器 dark theme 后把时间轴深色背景误判为贴边文字，并将右侧价格轴上限收紧到 72px。
