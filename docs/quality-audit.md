...
补充：阶段 1 研究页、回测详情和交易详情的 K 线图表布局在 2026-07-01 以 `tictickbot` / `tictick-lite` 的正常 K 线比例重新校准；当前有效布局约束以 `klineChartLayout.css`、`ResearchPage.css`、`detailChartLayout.css`、`scripts/stage8-visual-smoke.mjs` 和 `scripts/research-chart-height-smoke.mjs` 为准：研究页主工具栏 symbol 输入为桌面/窄桌面 `96px`、移动端 `92px`，桌面工具栏采用左侧 market strip + 右侧单行可滚动状态摘要的一行工作台布局，窄屏再堆叠；图表左/右 gutter 为桌面 `14px/2px`、窄桌面 `12px/2px`、移动端 `10px/2px`；plot 高度为桌面 `clamp(680px, 72dvh, 820px)`、窄桌面 `700px`、移动端 `580px`，上下 padding 归零；共享 `TradingViewChart` 坐标轴字号收敛为桌面/窄桌面 `14px`、移动端 `13px`，右侧价格轴 minimumWidth 为桌面 `78px`、窄桌面 `74px`、移动端 `68px`，价格 formatter 继续输出完整数值且禁止 `k/K/m/M/b/B` 缩写；visual smoke 断言 symbol 最大宽度 `100px`、工具栏控件最大宽度 `500px`、工具栏高度最大 `72px`、右侧价格轴最大宽度 `84px`、坐标轴文字墨迹高度范围为桌面/窄桌面 `9px` 到 `16px`、移动端 `9px` 到 `14px`、主图 canvas 右边界贴住右侧价格轴左边界、最右侧 canvas 贴住 viewport 右边界，详情页下方摘要列保持 `minmax(220px, 260px)`。

补充：阶段 1 K 线图表坐标轴正常范围在 2026-07-01 继续收敛；本段覆盖前述过小和巨型字号旧合同。参考 `tictickbot` / `tictick-lite` 未显式放大 lightweight-charts 轴字号的比例，当前有效合同为共享 `TradingViewChart` 坐标轴字号收敛为桌面/窄桌面 `14px`、移动端 `13px`，右侧价格轴 minimumWidth 为桌面 `78px`、窄桌面 `74px`、移动端 `68px`，价格 formatter 继续输出完整数值且禁止 `k/K/m/M/b/B` 缩写；visual smoke 仍要求右侧价格轴最大宽度 `84px`、主图宽度占比下限桌面 `0.90` / 窄桌面 `0.82` / 移动端 `0.70`，并把坐标轴文字墨迹高度范围为桌面/窄桌面 `9px` 到 `16px`、移动端 `9px` 到 `14px`，过小和过大的坐标轴都会失败。该证据只修正 K 线图表视觉合同，不改变 CandleProvider、data sync、回测、交易或实盘能力等级。
