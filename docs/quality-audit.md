...
- 如果批次只返回未闭合 K 线，runner 不写入 `market_candles`、不推进 cursor，也不会把有界同步任务误判为 completed。
- 单元测试覆盖混合闭合/未闭合批次只保存闭合 K 线，以及只有未闭合 K 线的批次不完成任务。
- PostgreSQL 集成测试覆盖真实 runner + store 路径下未闭合 K 线不会落库，cursor 只推进到最后一根闭合 K 线，任务保持 pending 等待后续闭合数据。
