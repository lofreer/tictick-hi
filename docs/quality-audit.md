...
- 审计列表仍缺 cursor pagination、导出、保留策略、不可篡改存储和归档策略。
...
- 数据库管理员仍可同时篡改数据和 hash，仍缺外部不可篡改存储、签名、保留、归档和导出。
