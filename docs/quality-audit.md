...
| CandleProvider | demo | native/aggregated/gap/coverage/pagination/window metadata、opaque adjacent-window cursor、runner 健康门禁和集成测试已覆盖 | 大范围性能压测、聚合窗口分页读取、异常数据修复策略不足 |
...
- 本轮只补 adjacent-window opaque cursor，不做聚合窗口跨多页读取、预取、虚拟化或大范围历史性能压测。
