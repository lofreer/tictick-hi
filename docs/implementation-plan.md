...
CSRF 写请求保护、可配置登录失败持久化节流、基础会话列表 / 来源上下文 / 撤销、基础认证审计、审计列表 cursor 分页 / CSV 导出 / hash 短码可观察 / hash chain 在线校验、操作员 username 非空白校验、基础密码长度 / 复杂度 / 弱密码字典 / 不含用户名策略、当前操作员自禁用保护和最后启用操作员保护已进入本地 demo 边界；更完整弱密码字典 / 历史 / 轮换、MFA、RBAC、角色级自保护规则和生产级设备上下文仍后续确认。
