...
- 数据实时同步第一版采用 `hi sync` REST 轮询；系统重启后必须立即恢复，继续同步对应交易所、交易对、周期的数据。
...
- 查看图表。
- 查看并修复任务窗口内缺口。
- 查看任务窗口内异常；错位 `open_time` 只能隔离，不能走普通补同步修复。
- 任务窗口错位 `open_time` 隔离成功后，应回到同一研究上下文打开缺口详情，引导用户显式排补同步任务，不能自动伪装修复成功。
- 删除。
...
- “实时”第一版表示 `hi sync` 按 `SYNC_POLL_INTERVAL` 持续 claim 任务并轮询交易所 K 线 REST 接口，让数据保持更新；不接入 WebSocket 订阅。
...
- 第一版前端统一暴露 `1m`、`5m`、`15m`、`1h`、`4h`、`1d`；其中 `1m` 作为基础周期，更高周期默认走后端聚合层。
...
- 对于实时同步任务，第一版必须根据已持久化的最新 K 线时间继续轮询拉取；WebSocket 订阅保留为后续交易所差异化能力。
- 不能因为 daemon 重启就丢失进度。
- 同步进度必须持久化。
...
- 能创建数据同步任务，且 exchange / symbol / interval 不能是纯空白。
- 能在研究页看到同步任务列表。
- 能启动 / 停止同步。
- 能启动 / 停止实时。
- 能从列表加载对应图表。
...
回测任务创建必须校验名称、交易所、交易对、周期和策略 ID 非空白。

回测第一版保留 bps 级手续费和滑点配置：

- `feeBps` 和 `slippageBps` 创建时必须是 `0..10000` 的数值。
- 买入按更差成交价加滑点，卖出按更差成交价减滑点。
- 买卖成交都从现金流扣手续费，并在结果摘要中记录 fee / slippage / total fees。
- 第一版仍不是生产级撮合曲线，不声明订单簿、盘口深度、部分成交或动态费率可信。
...
交易任务创建必须校验名称、类型、交易所、交易对、周期、策略 ID 和账号非空白。
...
- 实盘任务创建确认文案必须明确这是实盘。
- 创建实盘任务必须二次输入 `LIVE`，且 API 必须在落库前校验该确认字段。
...
当前操作员修改密码已进入本地 demo 边界：要求当前密码和新密码，复用基础密码策略，成功后保留当前会话并撤销同账号其它会话。
...
CSRF 写请求保护、可配置登录失败持久化节流、基础会话列表 / 来源上下文 / IP-UA 变化提示与 24h 去重 warning 审计 / 撤销、当前操作员修改密码 / 其它会话吊销、禁用操作员撤销该账号既有 session 并在成功审计记录撤销数量、管理员独立撤销指定 operator 全部 session 并在成功审计记录撤销数量、管理员查看并精确撤销指定 operator 单个 session、管理员重置指定 operator 密码并撤销该账号 sessions、可配置最近密码历史复用保护、基础认证审计、审计列表 cursor 分页 / CSV 导出 / hash 短码可观察 / hash chain 在线校验和页面入口、操作员 username 非空白校验、基础密码长度 / 复杂度 / 扩展弱密码字典 / 不含用户名策略、当前操作员自禁用保护、最后启用操作员保护、admin/operator 基础角色、最后启用 admin 保护、admin-only 角色变更、当前操作员自角色变更保护、当前操作员自 session 全量撤销保护、当前 session 精确撤销保护、角色变更审计、operator 列表读取、系统通知读取 / retry / channel 管理、交易所账号读取 / 创建、审计读取 / 导出 / hash chain 校验和 api-contract 读取已进入本地 demo 边界；定期轮换 / 自助忘记密码或账号恢复、MFA、细粒度 RBAC、多人审批和生产级设备上下文仍后续确认。
...
PUT  /api/system/notifications/channels/:id
DELETE /api/system/notifications/channels/:id
POST /api/system/notifications/channels/:id/enable
POST /api/system/notifications/channels/:id/disable
...
POST /api/system/operators/:id/password
GET  /api/system/operators/:id/sessions
DELETE /api/system/operators/:id/sessions/:sessionId
